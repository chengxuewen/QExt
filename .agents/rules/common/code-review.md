...
- [ ] Errors handled explicitly: no silently swallowed errors in changed files
- [ ] No hardcoded secrets: run `scripts/scan-hardcode.sh` or `/security-hardening`
- [ ] No qDebug noise in release code: `grep -rn 'qDebug()\|qWarning()' src/` reviewed
- [ ] Tests exist: ctest target builds and runs
- [ ] Coverage >=80%: lcov/gcovr (视 CI 配置)
