...
QDesignerFormWindow::QDesignerFormWindow(QExtDesignerAbstractFormWindow *editor, QDesignerWorkbench *workbench, QWidget *parent, Qt::WindowFlags flags)
      ...
      m_windowTitleInitialized(false)
{
    ...
    QExtDesignerAbstractFormEditor *core = workbench->core();
    ...
    connect(m_editor.data(), &QExtDesignerAbstractFormWindow::geometryChanged,
...
QExtDesignerAbstractFormWindow *QDesignerFormWindow::editor() const
{
    return m_editor;
}
...
void QDesignerFormWindow::firstShow()
{
    ...
    if (!m_windowTitleInitialized) {
        ...
        if (m_editor) {
            connect(m_editor.data(), &QExtDesignerAbstractFormWindow::fileNameChanged,
...
void QDesignerFormWindow::slotGeometryChanged()
{
    ...
    const QExtDesignerAbstractFormEditor *core = m_editor->core();
    QObject *object = core->propertyEditor()->object();
    if (object == nullptr || !object->isWidgetType())
        return;
    static const QString geometryProperty = QStringLiteral("geometry");
    const QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core->extensionManager(), object);
