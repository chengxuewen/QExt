...
static QStringList objectNameList(QExtDesignerAbstractFormWindow *form)
{
    QStringList result;
    ...
    if (const QExtDesignerContainerExtension *c = qt_extension<QExtDesignerContainerExtension *>(form->core()->extensionManager(), mainContainer)) {
        const int count = c->count();
        for (int i = 0 ; i < count; i++)
            addWidgetToObjectList(c->widget(i), result);
    }

    const QExtDesignerFormWindowCursorInterface *cursor = form->cursor();
    const int widgetCount = cursor->widgetCount();
    for (int i = 0; i < widgetCount; ++i)
        addWidgetToObjectList(cursor->widget(i), result);

    const QExtDesignerAbstractMetaDataBase *mdb = form->core()->metaDataBase();
...
bool ConnectionModel::setData(const QModelIndex &index, const QVariant &data, int)
{
    ...
    QExtDesignerAbstractFormWindow *form = m_editor->formWindow();
...
void ConnectionModel::connectionChanged(Connection *con)
{
    ...
    for (int i=0; i<m_editor->connectionCount(); ++i) {
        ...
        if (c->sender() == changedCon->sender() && c->signal() == changedCon->signal()
            ...
            m_editor->formWindow()->core()->dialogGui()->message(m_editor->parentWidget(), QExtDesignerAbstractDialogGui::SignalSlotEditorMessage,
...
public:
    ...
    void setForm(QExtDesignerAbstractFormWindow *form);
...
private:
    QExtDesignerAbstractFormWindow *m_form;
};
...
void ConnectionDelegate::setForm(QExtDesignerAbstractFormWindow *form)
{
    m_form = form;
}
...
SignalSlotEditorWindow::SignalSlotEditorWindow(QExtDesignerAbstractFormEditor *core,
                                                QWidget *parent)  :
    QWidget(parent),
    ...
    m_handling_selection_change(false)
{
    ...
    connect(core->formWindowManager(),
            &QExtDesignerAbstractFormWindowManager::activeFormWindowChanged,
            this, &SignalSlotEditorWindow::setActiveFormWindow);

    updateUi();
}

void SignalSlotEditorWindow::setActiveFormWindow(QExtDesignerAbstractFormWindow *form)
{
    QExtDesignerAbstractIntegration *integration = m_core->integration();
    ...
    if (!m_editor.isNull()) {
        ...
        if (integration) {
            disconnect(integration, &QExtDesignerAbstractIntegration::objectNameChanged,
    ...
    if (!m_editor.isNull()) {
        ...
        connect(m_editor.data(), &SignalSlotEditor::connectionAdded,
                this, &SignalSlotEditorWindow::resizeColumns);
        if (integration) {
            connect(integration, &QExtDesignerAbstractIntegration::objectNameChanged,
                    this, &SignalSlotEditorWindow::objectNameChanged);
        }
    }
...
void SignalSlotEditorWindow::objectNameChanged(QExtDesignerAbstractFormWindow *, QObject *, const QString &, const QString &)
{
    if (m_editor)
        m_model->updateAll();
}
