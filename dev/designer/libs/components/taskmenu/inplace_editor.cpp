...
{
    m_formWindow = QExtDesignerAbstractFormWindow::findFormWindow(m_widget);
    if (m_formWindow.isNull())
        return;
    m_managed = m_formWindow->isManaged(m_widget);
    // Close as soon as a different widget is selected
    connect(m_formWindow.data(), &QExtDesignerAbstractFormWindow::selectionChanged,
            this, &TaskMenuInlineEditor::updateSelection);

    // get old value
    QExtDesignerAbstractFormEditor *core = m_formWindow->core();
    const QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core->extensionManager(), m_widget);
