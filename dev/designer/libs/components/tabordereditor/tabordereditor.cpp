...
TabOrderEditor::TabOrderEditor(QExtDesignerAbstractFormWindow *form, QWidget *parent) :
    QWidget(parent),
    m_form_window(form),
    m_bg_widget(nullptr),
    ...
    m_beginning(true)
{
    connect(form, &QExtDesignerAbstractFormWindow::widgetRemoved, this, &TabOrderEditor::widgetRemoved);
...
QExtDesignerAbstractFormWindow *TabOrderEditor::formWindow() const
{
...
bool TabOrderEditor::skipWidget(QWidget *w) const
{
    ...
    if (const QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(ext, w)) {
...
void TabOrderEditor::initTabOrder()
{
    ...
    if (const QExtDesignerMetaDataBaseItemInterface *item = core->metaDataBase()->item(formWindow())) {
...
void TabOrderEditor::mousePressEvent(QMouseEvent *e)
{
    ...
    if (!m_indicator_region.contains(e->pos())) {
        if (QWidget *child = m_bg_widget->childAt(e->pos())) {
            QExtDesignerAbstractFormEditor *core = m_form_window->core();
