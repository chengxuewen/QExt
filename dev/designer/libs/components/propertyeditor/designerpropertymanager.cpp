...
class TextEditor : public QWidget
{
    Q_OBJECT
public:
    TextEditor(QExtDesignerAbstractFormEditor *core, QWidget *parent);
...
TextEditor::TextEditor(QExtDesignerAbstractFormEditor *core, QWidget *parent) :
    QWidget(parent),
...
class PixmapEditor : public QWidget
{
    Q_OBJECT
public:
    PixmapEditor(QExtDesignerAbstractFormEditor *core, QWidget *parent);
...
PixmapEditor::PixmapEditor(QExtDesignerAbstractFormEditor *core, QWidget *parent) :
    QWidget(parent),
    m_iconThemeModeEnabled(false),
    m_core(core),
...
DesignerPropertyManager::DesignerPropertyManager(QExtDesignerAbstractFormEditor *core, QObject *parent) :
...
DesignerEditorFactory::DesignerEditorFactory(QExtDesignerAbstractFormEditor *core, QObject *parent) :
...
ResetDecorator::ResetDecorator(const QExtDesignerAbstractFormEditor *core, QObject *parent)
    : QObject(parent)
    , m_spacing(-1)
    , m_core(core)
{
...
static inline bool isModifiedInMultiSelection(const QExtDesignerAbstractFormEditor *core,
                                              const QString &propertyName)
{
    const QExtDesignerAbstractFormWindow *form = core->formWindowManager()->activeFormWindow();
    if (!form)
        return false;
    ...
    for (int i = 0; i < selectionSize; ++i) {
        const QExtDesignerPropertySheetExtension *sheet =
            qt_extension<QExtDesignerPropertySheetExtension*>(core->extensionManager(),
