...
struct Property {
    Property() = default;
    Property(QExtDesignerPropertySheetExtension *sheet, int id)
        : m_sheet(sheet), m_id(id) {}

    QExtDesignerPropertySheetExtension *m_sheet{nullptr};
    int m_id{-1};
};
...
struct ItemViewPropertySheetPrivate {
    ItemViewPropertySheetPrivate(QExtDesignerAbstractFormEditor *core,
    ...
    QHash<QHeaderView *, QExtDesignerPropertySheetExtension *> m_propertySheet;
...
ItemViewPropertySheetPrivate::ItemViewPropertySheetPrivate(QExtDesignerAbstractFormEditor *core,
                                                           QHeaderView *horizontalHeader,
                                                           QHeaderView *verticalHeader)
{
    if (horizontalHeader)
        m_propertySheet.insert(horizontalHeader,
                               qt_extension<QExtDesignerPropertySheetExtension*>
...
{
    QExtDesignerPropertySheetExtension *headerSheet = d->m_propertySheet.value(hv);
...
bool ItemViewPropertySheet::reset(int index)
{
    const FakePropertyMap::iterator it = d->m_propertyIdMap.find(index);
    if (it != d->m_propertyIdMap.end()) {
       QExtDesignerPropertySheetExtension *headerSheet = it.value().m_sheet;
