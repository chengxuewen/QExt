...
void QMdiAreaPropertySheet::setProperty(int index, const QVariant &value)
{
    ...
    case MdiAreaSubWindowTitle:        // Forward to window title of subwindow
        if (QExtDesignerPropertySheetExtension *cws = currentWindowSheet()) {
...
{
    if (const QExtDesignerContainerExtension *c = qt_extension<QExtDesignerContainerExtension*>(core()->extensionManager(), object())) {
...
QExtDesignerPropertySheetExtension *QMdiAreaPropertySheet::currentWindowSheet() const
{
    QWidget *cw = currentWindow();
    if (cw == nullptr)
        return nullptr;
    return qt_extension<QExtDesignerPropertySheetExtension*>(core()->extensionManager(), cw);
}
