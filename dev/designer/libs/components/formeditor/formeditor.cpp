...
FormEditor::FormEditor(QObject *parent)
    : QExtDesignerAbstractFormEditor(parent)
{
    ...
    connect(formWindowManager, &QExtDesignerAbstractFormWindowManager::activeFormWindowChanged,
            widgetFactory, &WidgetFactory::activeFormWindowChanged);

    QExtensionManager *mgr = new QExtensionManager(this);
    const QString containerExtensionId = Q_TYPEID(QExtDesignerContainerExtension);
    ...
    mgr->registerExtensions(new QDesignerLayoutDecorationFactory(mgr),
                            Q_TYPEID(QExtDesignerLayoutDecorationExtension));
    ...
    mgr->registerExtensions(new QDesignerMemberSheetFactory(mgr),
                            Q_TYPEID(QExtDesignerMemberSheetExtension));
    ...
    QList<QExtDesignerOptionsPageInterface*> optionsPages;
...
void FormEditor::slotQrcFileChangedExternally(const QString &path)
{
    ...
    QExtDesignerIntegration::ResourceFileWatcherBehaviour behaviour = integration()->resourceFileWatcherBehaviour();
    if (behaviour == QExtDesignerIntegration::NoResourceFileWatcher)
        return;
    if (behaviour == QExtDesignerIntegration::PromptToReloadResourceFile) {
        QMessageBox::StandardButton button = dialogGui()->message(topLevel(), QExtDesignerAbstractDialogGui::FileChangedMessage, QMessageBox::Warning,
