...
QExtDesignerAbstractFormEditor *FormWindow::core() const
{
    return m_core;
}

QExtDesignerFormWindowCursorInterface *FormWindow::cursor() const
{
    return m_cursor;
}
...
void FormWindow::init()
{
    ...
    connect(m_widgetStack, &FormWindowWidgetStack::currentToolChanged,
            this, &QExtDesignerAbstractFormWindow::toolChanged);
    ...
    connect(m_geometryChangedTimer, &QTimer::timeout,
            this, &QExtDesignerAbstractFormWindow::geometryChanged);
    ...
    connect(&m_undoStack, &QUndoStack::indexChanged,
            this, &QExtDesignerAbstractFormWindow::changed);
    connect(&m_undoStack, &QUndoStack::cleanChanged,
            this, &FormWindow::slotCleanChanged);
    connect(this, &QExtDesignerAbstractFormWindow::changed,
...
void FormWindow::setMainContainer(QWidget *w)
{
    ...
    if (QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core()->extensionManager(), m_mainContainer)) {
...
static inline void clearObjectInspectorSelection(const QExtDesignerAbstractFormEditor *core)
{
...
static QWidget *findSelectedParent(QExtDesignerAbstractFormWindow *fw, const QWidget *w, bool selected)
{
    const QExtDesignerFormWindowCursorInterface *cursor = fw->cursor();
...
static bool canDragWidgetInLayout(const QExtDesignerAbstractFormEditor *core, QWidget *w)
{
    bool managed;
...
bool FormWindow::handleMouseMoveEvent(QWidget *, QWidget *, QMouseEvent *e)
{
    ...
    QExtDesignerFormWindowCursorInterface *c = cursor();
    ...
    QList<QExtDesignerDnDItemInterface*> item_list;
    const QPoint globalPos = mapToGlobal(m_startPos);
    const QExtDesignerDnDItemInterface::DropType dropType = (mouseFlags(e->modifiers()) & CopyDragModifier) ?
                            QExtDesignerDnDItemInterface::CopyDrop : QExtDesignerDnDItemInterface::MoveDrop;
    for (QWidget *widget : qAsConst(sel)) {
        item_list.append(new FormWindowDnDItem(dropType,  this, widget, globalPos));
        if (dropType == QExtDesignerDnDItemInterface::MoveDrop) {
    ...
    if (!sel.isEmpty()) // reshow selection?
        if (QDesignerMimeData::execDrag(item_list, core()->topLevel()) == Qt::IgnoreAction && dropType == QExtDesignerDnDItemInterface::MoveDrop)
            for (QWidget *widget : qAsConst(sel))
...
void FormWindow::ensureUniqueObjectName(QObject *object)
{
    ...
    if (name.isEmpty()) {
        ...
        if (QExtDesignerWidgetDataBaseItemInterface *item = db->item(db->indexOfObject(object)))
...
static inline void insertNames(const QExtDesignerAbstractMetaDataBase *metaDataBase,
...
bool FormWindow::unify(QObject *w, QString &s, bool changeIt)
{
    ...
    const QExtDesignerAbstractMetaDataBase *metaDataBase = core()->metaDataBase();
...
bool FormWindow::handleKeyPressEvent(QWidget *widget, QWidget *, QKeyEvent *e)
{
    ...
    switch (e->key()) {
        ...
        case Qt::Key_Tab:
            if (e->modifiers() == Qt::NoModifier)
                cursor()->movePosition(QExtDesignerFormWindowCursorInterface::Next);
            break;

        case Qt::Key_Backtab:
            if (e->modifiers() == Qt::NoModifier)
                cursor()->movePosition(QExtDesignerFormWindowCursorInterface::Prev);
            break;
...
public:
    ArrowKeyPropertyHelper(QObject* o, SpecialProperty sp,
                       QExtDesignerPropertySheetExtension *s, int i) :
                       PropertyHelper(o, sp, s, i) {}

    Value setValue(QExtDesignerAbstractFormWindow *fw, const QVariant &value, bool changed, unsigned subPropertyMask) override;
};

PropertyHelper::Value ArrowKeyPropertyHelper::setValue(QExtDesignerAbstractFormWindow *fw, const QVariant &value, bool changed, unsigned subPropertyMask)
{
...
public:
    explicit ArrowKeyPropertyCommand(QExtDesignerAbstractFormWindow *fw,
...
protected:
    PropertyHelper *createPropertyHelper(QObject *o, SpecialProperty sp,
                                         QExtDesignerPropertySheetExtension *s, int i) const override
        { return new ArrowKeyPropertyHelper(o, sp, s, i); }
    QVariant mergeValue(const QVariant &newValue) override;
};

ArrowKeyPropertyCommand::ArrowKeyPropertyCommand(QExtDesignerAbstractFormWindow *fw,
                                                 QUndoCommand *p) :
...
{
    const QExtDesignerFormWindowCursorInterface *c = cursor();
    if (!c->hasSelection())
        return;
...
QWidget *FormWindow::innerContainer(QWidget *outerContainer) const
{
    if (m_core->widgetDataBase()->isContainer(outerContainer))
        if (const QExtDesignerContainerExtension *container = qt_extension<QExtDesignerContainerExtension*>(m_core->extensionManager(), outerContainer)) {
...
void FormWindow::paste(PasteMode pasteMode)
{
    ...
    do {
        ...
        if (widgetCount && pasteContainer == nullptr) {
            ...
            core()->dialogGui()->message(this, QExtDesignerAbstractDialogGui::FormEditorMessage, QMessageBox::Information,
...
void FormWindow::breakLayout(QWidget *w)
{
    ...
    const QExtDesignerAbstractMetaDataBase *mdb = core()->metaDataBase();
...
QMenu *FormWindow::createPopupMenu(QWidget *w)
{
    ...
    QExtDesignerAbstractFormWindowManager *manager = core()->formWindowManager();
    ...
    if (!isFormWindow) {
        ...
        if (manager->action(QExtDesignerAbstractFormWindowManager::LowerAction)->isEnabled()) {
            popup->addAction(manager->action(QExtDesignerAbstractFormWindowManager::LowerAction));
            popup->addAction(manager->action(QExtDesignerAbstractFormWindowManager::RaiseAction));
            popup->addSeparator();
        }
#if QT_CONFIG(clipboard)
        popup->addAction(manager->action(QExtDesignerAbstractFormWindowManager::CutAction));
        popup->addAction(manager->action(QExtDesignerAbstractFormWindowManager::CopyAction));
#endif
    }

#if QT_CONFIG(clipboard)
    popup->addAction(manager->action(QExtDesignerAbstractFormWindowManager::PasteAction));
#endif

    if (QAction *selectAncestorAction = createSelectAncestorSubMenu(w))
        popup->addAction(selectAncestorAction);
    popup->addAction(manager->action(QExtDesignerAbstractFormWindowManager::SelectAllAction));

    if (!isFormWindow) {
        popup->addAction(manager->action(QExtDesignerAbstractFormWindowManager::DeleteAction));
    }

    popup->addSeparator();
    QMenu *layoutMenu = popup->addMenu(tr("Lay out"));
    layoutMenu->addAction(manager->action(QExtDesignerAbstractFormWindowManager::AdjustSizeAction));
    layoutMenu->addAction(manager->action(QExtDesignerAbstractFormWindowManager::HorizontalLayoutAction));
    layoutMenu->addAction(manager->action(QExtDesignerAbstractFormWindowManager::VerticalLayoutAction));
    if (!isFormWindow) {
        layoutMenu->addAction(manager->action(QExtDesignerAbstractFormWindowManager::SplitHorizontalAction));
        layoutMenu->addAction(manager->action(QExtDesignerAbstractFormWindowManager::SplitVerticalAction));
    }
    layoutMenu->addAction(manager->action(QExtDesignerAbstractFormWindowManager::GridLayoutAction));
    layoutMenu->addAction(manager->action(QExtDesignerAbstractFormWindowManager::FormLayoutAction));
    layoutMenu->addAction(manager->action(QExtDesignerAbstractFormWindowManager::BreakLayoutAction));
    layoutMenu->addAction(manager->action(QExtDesignerAbstractFormWindowManager::SimplifyLayoutAction));
...
{
    if (QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core()->extensionManager(), w)) {
...
QWidget *FormWindow::findContainer(QWidget *w, bool excludeLayout) const
{
    ...
    QExtDesignerAbstractWidgetFactory *widgetFactory = core()->widgetFactory();
...
{
    return qobject_cast<FormWindow*>(QExtDesignerAbstractFormWindow::findFormWindow(w));
}
...
void FormWindow::highlightWidget(QWidget *widget, const QPoint &pos, HighlightMode mode)
{
    ...
    } else if (QExtDesignerLayoutDecorationExtension *g = qt_extension<QExtDesignerLayoutDecorationExtension*>(core()->extensionManager(), container)) {
        if (mode == Restore) {
...
QExtDesignerAbstractFormWindowTool *FormWindow::tool(int index) const
{
    return m_widgetStack->tool(index);
}

void FormWindow::registerTool(QExtDesignerAbstractFormWindowTool *tool)
{
...
bool FormWindow::handleEvent(QWidget *widget, QWidget *managedWidget, QEvent *event)
{
    ...
    QExtDesignerAbstractFormWindowTool *tool = m_widgetStack->currentTool();
    if (tool == nullptr)
        return false;
...
void FormWindow::dragWidgetWithinForm(QWidget *widget, const QRect &targetGeometry, QWidget *targetContainer)
{
    ...
    const QExtDesignerLayoutDecorationExtension *targetDeco = qt_extension<QExtDesignerLayoutDecorationExtension*>(core()->extensionManager(), targetContainer);
...
bool FormWindow::dropDockWidget(QExtDesignerDnDItemInterface *item, const QPoint &global_mouse_pos)
{
    ...
    QExtDesignerPropertySheetExtension *propertySheet = qobject_cast<QExtDesignerPropertySheetExtension*>(m_core->extensionManager()->extension(widget, Q_TYPEID(QExtDesignerPropertySheetExtension)));
    if (propertySheet) {
...
bool FormWindow::dropWidgets(const QList<QExtDesignerDnDItemInterface*> &item_list, QWidget *target,
                             const QPoint &global_mouse_pos)
{
    ...
    QPoint offset;
    QExtDesignerDnDItemInterface *current = nullptr;
    QExtDesignerFormWindowCursorInterface *c = cursor();
    for (QExtDesignerDnDItemInterface *item : qAsConst(item_list)) {
    ...
    for (QExtDesignerDnDItemInterface *item : qAsConst(item_list)) {
        ...
        if (item->type() == QExtDesignerDnDItemInterface::CopyDrop) { // from widget box or CTRL + mouse move
            ...
            QExtDesignerAbstractFormWindow *dest = findFormWindow(widget);
            if (dest == this) {
