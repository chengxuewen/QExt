...
public:
    ...
    explicit WidgetBoxCategoryListView(QExtDesignerAbstractFormEditor *core, QWidget *parent = nullptr);
    void setViewMode(ViewMode vm);

    void dropWidgets(const QList<QExtDesignerDnDItemInterface*> &item_list);

    using QListView::contentsSize;

    // These methods operate on the filtered/unfiltered model according to accessmode
    int count(AccessMode am) const;
    QExtDesignerAbstractWidgetBox::Widget widgetAt(AccessMode am, const QModelIndex &index) const;
    QExtDesignerAbstractWidgetBox::Widget widgetAt(AccessMode am, int row) const;
    ...
    static QString widgetDomXml(const QExtDesignerAbstractWidgetBox::Widget &widget);
