...
struct WidgetBoxCategoryEntry {
    ...
    explicit WidgetBoxCategoryEntry(const QExtDesignerAbstractWidgetBox::Widget &widget,
...
WidgetBoxCategoryEntry::WidgetBoxCategoryEntry(const QExtDesignerAbstractWidgetBox::Widget &w,
...
public:
    explicit WidgetBoxCategoryModel(QExtDesignerAbstractFormEditor *core, QObject *parent = nullptr);
    ...
    void addWidget(const QExtDesignerAbstractWidgetBox::Widget &widget, const QIcon &icon, bool editable);

    QExtDesignerAbstractWidgetBox::Widget widgetAt(const QModelIndex & index) const;
    QExtDesignerAbstractWidgetBox::Widget widgetAt(int row) const;

    int indexOfWidget(const QString &name);

    QExtDesignerAbstractWidgetBox::Category category() const;
...
WidgetBoxCategoryModel::WidgetBoxCategoryModel(QExtDesignerAbstractFormEditor *core, QObject *parent) :
...
QExtDesignerAbstractWidgetBox::Category WidgetBoxCategoryModel::category() const
{
    QExtDesignerAbstractWidgetBox::Category rc;
...
bool WidgetBoxCategoryModel::removeCustomWidgets()
{
    ...
    for (WidgetBoxCategoryEntrys::iterator it = m_items.begin(); it != m_items.end(); )
        if (it->widget.type() == QExtDesignerAbstractWidgetBox::Widget::Custom) {
            if (!changed)
                beginResetModel();
            it = m_items.erase(it);
            changed = true;
...
void WidgetBoxCategoryModel::addWidget(const QExtDesignerAbstractWidgetBox::Widget &widget, const QIcon &icon,bool editable)
{
    ...
    if (dbIndex != -1) {
        const QExtDesignerWidgetDataBaseItemInterface *dbItem = db->item(dbIndex);
...
QExtDesignerAbstractWidgetBox::Widget WidgetBoxCategoryModel::widgetAt(const QModelIndex & index) const
{
    return widgetAt(index.row());
}

QExtDesignerAbstractWidgetBox::Widget WidgetBoxCategoryModel::widgetAt(int row) const
{
    if (row < 0 || row >=  m_items.size())
        return QExtDesignerAbstractWidgetBox::Widget();
...
WidgetBoxCategoryListView::WidgetBoxCategoryListView(QExtDesignerAbstractFormEditor *core, QWidget *parent) :
    QListView(parent),
...
{
    const QExtDesignerAbstractWidgetBox::Widget wgt = m_model->widgetAt(m_proxyModel->mapToSource(index));
    if (wgt.isNull())
        return;
...
QExtDesignerAbstractWidgetBox::Widget WidgetBoxCategoryListView::widgetAt(AccessMode am, const QModelIndex & index) const
{
...
QExtDesignerAbstractWidgetBox::Widget WidgetBoxCategoryListView::widgetAt(AccessMode am, int row) const
{
...
void WidgetBoxCategoryListView::addWidget(const QExtDesignerAbstractWidgetBox::Widget &widget, const QIcon &icon, bool editable)
{
    m_model->addWidget(widget, icon, editable);
}

QString WidgetBoxCategoryListView::widgetDomXml(const QExtDesignerAbstractWidgetBox::Widget &widget)
{
...
QExtDesignerAbstractWidgetBox::Category WidgetBoxCategoryListView::category() const
{
