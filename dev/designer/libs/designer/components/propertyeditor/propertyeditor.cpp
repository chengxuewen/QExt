...
QExtDesignerMetaDataBaseItemInterface* PropertyEditor::metaDataBaseItem() const
{
    QObject *o = object();
    if (!o)
        return nullptr;
    QExtDesignerAbstractMetaDataBase *db = core()->metaDataBase();
    if (!db)
        return nullptr;
    return db->item(o);
}
...
PropertyEditor::PropertyEditor(QExtDesignerAbstractFormEditor *core, QWidget *parent, Qt::WindowFlags flags)  :
...
void PropertyEditor::slotAddDynamicProperty(QAction *action)
{
    ...
    const QExtDesignerDynamicPropertySheetExtension *dynamicSheet =
            qt_extension<QExtDesignerDynamicPropertySheetExtension*>(m_core->extensionManager(), m_object);
...
QExtDesignerAbstractFormEditor *PropertyEditor::core() const
{
    return m_core;
}
...
void PropertyEditor::updateBrowserValue(QtVariantProperty *property, const QVariant &value)
{
    ...
    QDesignerPropertySheet *sheet = qobject_cast<QDesignerPropertySheet*>(m_core->extensionManager()->extension(m_object, Q_TYPEID(QExtDesignerPropertySheetExtension)));
    int index = -1;
    if (sheet)
...
QString PropertyEditor::realClassName(QObject *object) const
{
    ...
    if (QExtDesignerWidgetDataBaseItemInterface *widgetItem = db->item(db->indexOfObject(object, true))) {
...
{
    QExtDesignerAbstractFormWindow *oldFormWindow = QExtDesignerAbstractFormWindow::findFormWindow(m_object);
    ...
    QExtDesignerAbstractFormWindow *formWindow = QExtDesignerAbstractFormWindow::findFormWindow(m_object);
    ...
    const QExtDesignerDynamicPropertySheetExtension *dynamicSheet =
            qt_extension<QExtDesignerDynamicPropertySheetExtension*>(m_core->extensionManager(), m_object);
    const QDesignerPropertySheet *sheet = qobject_cast<QDesignerPropertySheet*>(m_core->extensionManager()->extension(m_object, Q_TYPEID(QExtDesignerPropertySheetExtension)));
    ...
    m_propertySheet = qobject_cast<QExtDesignerPropertySheetExtension*>(m->extension(object, Q_TYPEID(QExtDesignerPropertySheetExtension)));
    if (m_propertySheet) {
    ...
    if (QWidget *widget = qobject_cast<QWidget*>(object)) {
        if (QExtDesignerAbstractFormWindow *fw = QExtDesignerAbstractFormWindow::findFormWindow(widget)) {
...
{
    QExtDesignerAbstractFormWindow *form = m_core->formWindowManager()->activeFormWindow();
    if (!form)
        return;
