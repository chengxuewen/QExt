...
{
public:
    explicit ModifyConnectionCommand(QExtDesignerAbstractFormWindow *form,
...
ModifyConnectionCommand::ModifyConnectionCommand(QExtDesignerAbstractFormWindow *form,
                                                 SignalSlotConnection *conn,
                                                 const QString &newSignal,
                                                 const QString &newSlot) :
...
SignalSlotEditor::SignalSlotEditor(QExtDesignerAbstractFormWindow *form_window, QWidget *parent) :
...
QWidget *SignalSlotEditor::widgetAt(const QPoint &pos) const
{
    ...
    for (; widget != nullptr; widget = widget->parentWidget()) {
        QExtDesignerMetaDataBaseItemInterface *item = m_form_window->core()->metaDataBase()->item(widget);
        if (item == nullptr)
            continue;
