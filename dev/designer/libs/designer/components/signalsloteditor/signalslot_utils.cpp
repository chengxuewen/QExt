...
static void memberList(QExtDesignerAbstractFormEditor *core,
                       QObject *object,
                       ...
                       OutputIterator it)
{
    if (!object)
        return;
    // 1) member sheet
    const QExtDesignerMemberSheetExtension *members = qt_extension<QExtDesignerMemberSheetExtension*>(core->extensionManager(), object);
...
namespace {
    ...
    public:
        SignalMatchesSlotPredicate(QExtDesignerAbstractFormEditor *core, const QString &peer, qdesigner_internal::MemberType memberType);
    ...
    SignalMatchesSlotPredicate::SignalMatchesSlotPredicate(QExtDesignerAbstractFormEditor *core, const QString &peer, qdesigner_internal::MemberType memberType) :
        m_peer(peer),
...
namespace qdesigner_internal {
    ...
    bool signalMatchesSlot(QExtDesignerAbstractFormEditor *core, const QString &signal, const QString &slot)
    {
    ...
    ClassesMemberFunctions reverseClassesMemberFunctions(const QString &obj_name, MemberType member_type,
                                                         const QString &peer, QExtDesignerAbstractFormWindow *form)
    {
        ...
        QExtDesignerAbstractFormEditor *core = form->core();
    ...
    QMap<QString, QString> getSignals(QExtDesignerAbstractFormEditor *core, QObject *object, bool showAll)
    {
    ...
    QMap<QString, QString> getMatchingSlots(QExtDesignerAbstractFormEditor *core, QObject *object, const QString &signalSignature, bool showAll)
    {
        QMap<QString, QString> rc;
    ...
    bool memberFunctionListContains(QExtDesignerAbstractFormEditor *core, QObject *object, MemberType type, const QString &signature)
    {
        QMap<QString, QString> rc;
    ...
    // ### deprecated
    QString realObjectName(QExtDesignerAbstractFormEditor *core, QObject *object)
    {
        if (!object)
            return QString();

        const QExtDesignerAbstractMetaDataBase *mdb = core->metaDataBase();
        if (const QExtDesignerMetaDataBaseItemInterface *item = mdb->item(object))
            return item->name();
