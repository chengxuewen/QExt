...
namespace qdesigner_internal {
    ...
    struct ModelRecursionContext {
        explicit ModelRecursionContext(QExtDesignerAbstractFormEditor *core, const QString &sepName);
        ...
        const QExtDesignerAbstractMetaDataBase *mdb;
    };

    ModelRecursionContext::ModelRecursionContext(QExtDesignerAbstractFormEditor *c, const QString &sepName) :
    ...
    void ObjectData::initWidget(QWidget *w, const ModelRecursionContext &ctx)
    {
        ...
        if (const QExtDesignerWidgetDataBaseItemInterface *widgetItem = ctx.db->item(ctx.db->indexOfObject(w, true))) {
        ...
        if (qt_extension<QExtDesignerContainerExtension*>(ctx.core->extensionManager(), w)) {
    ...
    void createModelRecursion(const QExtDesignerAbstractFormWindow *fwi,
                              QObject *parent,
                              QObject *object,
                              ObjectModel &model,
                              const ModelRecursionContext &ctx)
    {
        ...
        const QExtDesignerContainerExtension *containerExtension = nullptr;
        if (entry.type() == ObjectData::ExtensionContainer) {
            containerExtension = qt_extension<QExtDesignerContainerExtension*>(fwi->core()->extensionManager(), object);
    ...
    ObjectInspectorModel::UpdateResult ObjectInspectorModel::update(QExtDesignerAbstractFormWindow *fw)
    {
