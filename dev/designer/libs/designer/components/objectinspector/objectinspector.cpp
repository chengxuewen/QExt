...
static inline SelectionType selectionType(const QExtDesignerAbstractFormWindow *fw, QObject *o)
{
    if (!o->isWidgetType())
...
public:
    ObjectInspectorPrivate(QExtDesignerAbstractFormEditor *core);
    ~ObjectInspectorPrivate();

    QLineEdit *filterLineEdit() const { return m_filterLineEdit; }
    QTreeView *treeView() const { return m_treeView; }
    QExtDesignerAbstractFormEditor *core() const { return m_core; }
    const QPointer<FormWindowBase> &formWindow() const { return m_formWindow; }

    void clear();
    void setFormWindow(QExtDesignerAbstractFormWindow *fwi);
...
private:
    void setFormWindowBlocked(QExtDesignerAbstractFormWindow *fwi);
...
ObjectInspector::ObjectInspectorPrivate::ObjectInspectorPrivate(QExtDesignerAbstractFormEditor *core) :
    m_core(core),
...
void ObjectInspector::ObjectInspectorPrivate::showContainersCurrentPage(QWidget *widget)
{
    ...
    while (w != nullptr) {
        if (fw->isManaged(w) && !qobject_cast<QMainWindow *>(w)) { // Rule out unmanaged internal scroll areas, for example, on QToolBoxes.
            if (QExtDesignerContainerExtension *c = qt_extension<QExtDesignerContainerExtension*>(m_core->extensionManager(), w)) {
...
static inline bool mainContainerIsCurrent(const QExtDesignerAbstractFormWindow *fw)
{
...
void ObjectInspector::ObjectInspectorPrivate::setFormWindow(QExtDesignerAbstractFormWindow *fwi)
{
...
void ObjectInspector::ObjectInspectorPrivate::setFormWindowBlocked(QExtDesignerAbstractFormWindow *fwi)
{
...
{
    const QExtDesignerFormWindowCursorInterface *cursor = m_formWindow->cursor();
...
static inline QMenu *createTaskMenu(QObject *object, QExtDesignerAbstractFormWindow *fw)
{
    // 1) Objects
    if (!object->isWidgetType())
...
// ------------ ObjectInspector
ObjectInspector::ObjectInspector(QExtDesignerAbstractFormEditor *core, QWidget *parent) :
...
QExtDesignerAbstractFormEditor *ObjectInspector::core() const
{
    return m_impl->core();
}
...
void ObjectInspector::setFormWindow(QExtDesignerAbstractFormWindow *fwi)
{
