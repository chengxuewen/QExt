...
void LayoutPropertySheet::stretchAttributesToDom(QExtDesignerAbstractFormEditor *core, QLayout *lt, DomLayout *domLayout)
{
    ...
    if (!(visibleMask & (LayoutProperties::BoxStretchProperty|LayoutProperties::GridColumnStretchProperty|LayoutProperties::GridRowStretchProperty)))
        return;
    const QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core->extensionManager(), lt);
    Q_ASSERT(sheet);
...
void LayoutPropertySheet::markChangedStretchProperties(QExtDesignerAbstractFormEditor *core, QLayout *lt, const DomLayout *domLayout)
{
    // While the actual values are applied by the form builder, we still need
    // to mark them as 'changed'.
    QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core->extensionManager(), lt);
    Q_ASSERT(sheet);
