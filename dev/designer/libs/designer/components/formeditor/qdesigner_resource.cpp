...
{
public:
    QDesignerResourceBuilder(QExtDesignerAbstractFormEditor *core, DesignerPixmapCache *pixmapCache, DesignerIconCache *iconCache);
...
private:
    ...
    const QExtDesignerLanguageExtension *m_lang;
    bool                  m_saveRelative;
    mutable QMap<QString, bool>   m_usedQrcFiles;
    mutable QMap<QString, bool>   m_loadedQrcFiles;
};

QDesignerResourceBuilder::QDesignerResourceBuilder(QExtDesignerAbstractFormEditor *core, DesignerPixmapCache *pixmapCache, DesignerIconCache *iconCache) :
    m_core(core),
    m_pixmapCache(pixmapCache),
    m_iconCache(iconCache),
    m_lang(qt_extension<QExtDesignerLanguageExtension *>(core->extensionManager(), core)),
    m_saveRelative(true)
{
}
...
static inline void setIconPixmap(QIcon::Mode m, QIcon::State s, const QDir &workingDirectory,
                                 ...
                                 const QExtDesignerLanguageExtension *lang = nullptr)
{
    ...
    m_resourceBuilder(new QDesignerResourceBuilder(m_formWindow->core(), m_formWindow->pixmapCache(), m_formWindow->iconCache()))
{
    ...
    if (const QExtDesignerLanguageExtension *le = qt_extension<QExtDesignerLanguageExtension*>(core->extensionManager(), core))
...
void QDesignerResource::saveDom(DomUI *ui, QWidget *widget)
{
    ...
    QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core()->extensionManager(), widget);
    ...
    for (int index = 0; index < m_formWindow->toolCount(); ++index) {
        QExtDesignerAbstractFormWindowTool *tool = m_formWindow->tool(index);
    ...
    if (QExtDesignerExtraInfoExtension *extra = qt_extension<QExtDesignerExtraInfoExtension*>(core()->extensionManager(), core()))
...
QWidget *QDesignerResource::create(DomUI *ui, QWidget *parentWidget)
{
    ...
    if (QExtDesignerExtraInfoExtension *extra = qt_extension<QExtDesignerExtraInfoExtension*>(core()->extensionManager(), core())) {
        if (!extra->loadUiExtraInfo(ui)) {
            ...
            core()->dialogGui()->message(parentWidget->window(), QExtDesignerAbstractDialogGui::FormLoadFailureMessage,
    ...
    QExtDesignerAbstractFormWindow *previousFormWindow = factory->currentFormWindow(m_formWindow);
    ...
    if (mainWidget && m_formWindow) {
        ...
        if (!mchildren.isEmpty()) {
            QExtDesignerAbstractMetaDataBase *mdb = core()->metaDataBase();
    ...
    if (mainWidget) {
        ...
        if (wdbIndex != -1) {
            QExtDesignerWidgetDataBaseItemInterface *item = wdb->item(wdbIndex);
...
QWidget *QDesignerResource::create(DomWidget *ui_widget, QWidget *parentWidget)
{
    ...
    if (!m_isMainWidget && className == QStringLiteral("QWidget")
        ...
        QExtDesignerContainerExtension *container = qt_extension<QExtDesignerContainerExtension*>(core()->extensionManager(), parentWidget);

        if (container == nullptr) {
            // generate a QLayoutWidget iff the parent is not an QExtDesignerContainerExtension.
...
QLayoutItem *QDesignerResource::create(DomLayoutItem *ui_layoutItem, QLayout *layout, QWidget *parentWidget)
{
    if (ui_layoutItem->kind() == DomLayoutItem::Spacer) {
        ...
        if (m_formWindow) {
            ...
            if (QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core()->extensionManager(), spacer))
...
static bool readDomEnumerationValue(const DomProperty *p,
                                    const QExtDesignerPropertySheetExtension* sheet, int index,
                                    QVariant &v)
{
    switch (p->kind()) {
...
void QDesignerResource::applyProperties(QObject *o, const QList<DomProperty*> &properties)
{
    ...
    QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core()->extensionManager(), o);
    if (!sheet)
        return;

    QExtDesignerDynamicPropertySheetExtension *dynamicSheet = qt_extension<QExtDesignerDynamicPropertySheetExtension*>(core()->extensionManager(), o);
...
QWidget *QDesignerResource::createWidget(const QString &widgetName, QWidget *parentWidget, const QString &_name)
{
    ...
    if (name.isEmpty()) {
        ...
        if (QExtDesignerWidgetDataBaseItemInterface *item = db->item(db->indexOfObject(w)))
...
DomWidget *QDesignerResource::createDom(QWidget *widget, DomWidget *ui_parentWidget, bool recursive)
{
    ...
    const QDesignerWidgetDataBaseInterface *wdb = core()->widgetDataBase();
    QExtDesignerWidgetDataBaseItemInterface *widgetInfo =  nullptr;
    ...
    if (widgetInfoIndex != -1) {
        ...
        QExtDesignerWidgetDataBaseItemInterface *customInfo = widgetInfo;
    ...
    else if (QExtDesignerContainerExtension *container = qt_extension<QExtDesignerContainerExtension*>(core()->extensionManager(), widget))
...
{
    QExtDesignerMetaDataBaseItemInterface *item = core()->metaDataBase()->item(layout);
...
{
    QExtDesignerMetaDataBaseItemInterface *item = core()->metaDataBase()->item(m_formWindow);
    Q_ASSERT(item);
 ...
 * to return 0. */
inline QString msgUnmanagedPage(QExtDesignerAbstractFormEditor *core,
...
DomWidget *QDesignerResource::saveWidget(QWidget *widget, QExtDesignerContainerExtension *container, DomWidget *ui_parentWidget)
{
...
DomWidget *QDesignerResource::saveWidget(QWizardPage *wizardPage, DomWidget *ui_parentWidget)
{
    ...
    QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core()->extensionManager(), wizardPage);
...
{
    const QExtDesignerMetaObjectInterface *meta = core()->introspection()->metaObject(obj);

    const int pindex = meta->indexOfProperty(prop);
    if (pindex != -1 && !(meta->property(pindex)->attributes(obj) & QExtDesignerMetaPropertyInterface::StoredAttribute))
        return false;
    ...
    if (QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core()->extensionManager(), obj)) {
...
bool QDesignerResource::addItem(DomWidget *ui_widget, QWidget *widget, QWidget *parentWidget)
{
    ...
    if (! QAbstractFormBuilder::addItem(ui_widget, widget, parentWidget) || qobject_cast<QMainWindow*> (parentWidget)) {
        if (QExtDesignerContainerExtension *container = qt_extension<QExtDesignerContainerExtension*>(core()->extensionManager(), parentWidget))
    ...
    if (QTabWidget *tabWidget = qobject_cast<QTabWidget*>(parentWidget)) {
        ...
        QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core()->extensionManager(), parentWidget);
...
DomCustomWidgets *QDesignerResource::saveCustomWidgets()
{
    ...
    for (auto it = m_usedCustomWidgets.cbegin(), end = m_usedCustomWidgets.cend(); it != end; ++it) {
        QExtDesignerWidgetDataBaseItemInterface *item = it.key();
...
{
    if (QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core()->extensionManager(), object)) {
...
QList<DomProperty*> QDesignerResource::computeProperties(QObject *object)
{
    ...
    if (QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(core()->extensionManager(), object)) {
        QExtDesignerDynamicPropertySheetExtension *dynamicSheet = qt_extension<QExtDesignerDynamicPropertySheetExtension*>(core()->extensionManager(), object);
...
DomProperty *QDesignerResource::applyProperStdSetAttribute(QObject *object, const QString &propertyName, DomProperty *property)
{
    ...
    if (const QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(mgr, object)) {
        const QExtDesignerDynamicPropertySheetExtension *dynamicSheet = qt_extension<QExtDesignerDynamicPropertySheetExtension*>(mgr, object);
        const QDesignerPropertySheet *designerSheet = qobject_cast<QDesignerPropertySheet*>(core()->extensionManager()->extension(object, Q_TYPEID(QExtDesignerPropertySheetExtension)));
        const int index = sheet->indexOf(propertyName);
...
static inline bool hasSetter(QExtDesignerAbstractFormEditor *core, QObject *object, const QString &propertyName)
{
    const QExtDesignerMetaObjectInterface *meta = core->introspection()->metaObject(object);
    ...
    if (resources != nullptr) {
        ...
        for (DomResource *res : dom_include) {
            ...
            while (!QFile::exists(path)) {
                ...
                const QMessageBox::StandardButton answer = core()->dialogGui()->message(dialogParent,  QExtDesignerAbstractDialogGui::ResourceLoadFailureMessage,
...
DomResources *QDesignerResource::saveResources()
{
    ...
    case QExtDesignerAbstractFormWindow::SaveAllResourceFiles:
        paths = m_formWindow->activeResourceFilePaths();
        break;
    case QExtDesignerAbstractFormWindow::SaveOnlyUsedResourceFiles:
        paths = m_resourceBuilder->usedQrcFiles();
        break;
    case QExtDesignerAbstractFormWindow::DontSaveResourceFiles:
        break;
    }
...
void QDesignerResource::applyAttributesToPropertySheet(const DomWidget *ui_widget, QWidget *widget)
{
    ...
    if (attributes.isEmpty())
        return;
    QExtDesignerPropertySheetExtension *sheet = qt_extension<QExtDesignerPropertySheetExtension*>(m_formWindow->core()->extensionManager(), widget);
