...
QEXTTcpPacketDispatcher::QEXTTcpPacketDispatcher(QObject *parent)
    : QEXTTcpPacketDispatcherBase(parent), QEXTObject(*(new QEXTTcpPacketDispatcherPrivate(this)))
{

}

QEXTTcpPacketDispatcher::QEXTTcpPacketDispatcher(QEXTTcpPacketDispatcherPrivate &dd, QObject *parent)
    : QEXTTcpPacketDispatcherBase(parent), QEXTObject(dd)
{
...
QEXTTcpPacketBase *QEXTTcpPacketDispatcher::dequeuePacket()
{
    QEXT_D(QEXTTcpPacketDispatcher);
    QMutexLocker mutexLocker(&d->m_mutex);
...
void QEXTTcpPacketDispatcher::enqueuePacket(QEXTTcpPacketBase *packet)
{
    if (packet) {
        QEXT_D(QEXTTcpPacketDispatcher);
        QMutexLocker mutexLocker(&d->m_mutex);
...
void QEXTTcpPacketDispatcher::dispatchPacket()
{
    ...
    while (0 != this->packetsCount()) {
        QEXTTcpPacketBase *packet = this->dequeuePacket();
        if (packet) {
            if (!d->m_tcpClient.isNull()) {
