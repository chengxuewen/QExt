#ifndef _QEXTBPABSTRACTGRAPHMODEL_H
#define _QEXTBPABSTRACTGRAPHMODEL_H
...
public:
     ...
     */
    virtual std::unordered_set<QExtBPTypes::ConnectionId> allConnectionIds(const QExtBPTypes::NodeId nodeId) const = 0;
     ...
     */
    virtual bool connectionExists(const QExtBPTypes::ConnectionId connectionId) const = 0;
     ...
     */
    virtual bool connectionPossible(const QExtBPTypes::ConnectionId connectionId) const = 0;

    /**
     * @brief Defines if detaching the connection is possible.
     */
    virtual bool detachPossible(const QExtBPTypes::ConnectionId) const { return true; }
     ...
     */
    virtual void addConnection(const QExtBPTypes::ConnectionId connectionId) = 0;

    /**
     * @returns `true` if there is data in the model associated with the
     * given `nodeId`.
     */
    virtual bool nodeExists(const QExtBPTypes::NodeId nodeId) const = 0;
     ...
     */
    virtual QVariant portData(QExtBPTypes::NodeId nodeId,
                              QExtBPTypes::PortTypeEnum portType,
                              QExtBPTypes::PortIndex index,
                              QExtBPTypes::PortRoleEnum role) const = 0;

    /**
     * A utility function that unwraps the `QVariant` value returned from the standard
     * `QVariant AbstractGraphModel::portData(...)` function.
     */
    template<typename T>
    T portData(QExtBPTypes::NodeId nodeId, QExtBPTypes::PortTypeEnum portType,
               QExtBPTypes::PortIndex index, QExtBPTypes::PortRoleEnum role) const
    {
    ...
    virtual bool deleteConnection(const QExtBPTypes::ConnectionId connectionId) = 0;

    virtual bool deleteNode(const QExtBPTypes::NodeId nodeId) = 0;

    /**
     * Reimplement the function if you want to store/restore the node's
     * inner state during undo/redo node deletion operations.
     */
    virtual QJsonObject saveNode(const QExtBPTypes::NodeId) const { return {}; }
...
public:
     ...
     */
    void portsAboutToBeDeleted(const QExtBPTypes::NodeId nodeId,
     ...
     */
    void portsAboutToBeInserted(const QExtBPTypes::NodeId nodeId,
                                const QExtBPTypes::PortTypeEnum portType,
                                const QExtBPTypes::PortIndex first,
                                const QExtBPTypes::PortIndex last);
     ...
     * After that the node is updated.
     */
    void portsInserted();

Q_SIGNALS:
    void connectionCreated(const QExtBPTypes::ConnectionId connectionId);

    void connectionDeleted(const QExtBPTypes::ConnectionId connectionId);

    void nodeCreated(const QExtBPTypes::NodeId nodeId);

    void nodeDeleted(const QExtBPTypes::NodeId nodeId);

    void nodeUpdated(const QExtBPTypes::NodeId nodeId);

    void nodeFlagsUpdated(const QExtBPTypes::NodeId nodeId);

    void nodePositionUpdated(const QExtBPTypes::NodeId nodeId);
...
private:
    QEXT_DECLARE_PRIVATE_D(dd_ptr, QExtBPAbstractGraphModel)
    QEXT_DECLARE_DISABLE_COPY_MOVE(QExtBPAbstractGraphModel)
};
