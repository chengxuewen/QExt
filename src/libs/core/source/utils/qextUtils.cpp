                      ...
                      std::vector<std::string>& vector)
{
    ...
    std::transform(list.begin(), list.end(), std::back_inserter(vector), &detail::qStringToSTLString);
}
...
//------------------------------------------------------------------------------
QSet<QString> qStringListToQSet(const QStringList &list)
{
...
//------------------------------------------------------------------------------
QStringList qSetToQStringList(const QSet<QString>& set)
{
#if (QT_VERSION >= QT_VERSION_CHECK(5, 14, 0))
...
//-----------------------------------------------------------------------------
int significantDecimals(double value, int defaultDecimals)
{
    if (value == 0.0 || qAbs(value) == std::numeric_limits<double>::infinity())
    {
        return 0;
    }
...
//-----------------------------------------------------------------------------
double closestPowerOfTen(double _value)
{
...
//-----------------------------------------------------------------------------
bool copyDirRecursively(const QString &srcPath, const QString &dstPath, bool includeHiddenFiles)
{
    // See http://stackoverflow.com/questions/2536524/copy-directory-using-qt
    if (!QFile::exists(srcPath))
    {
        qCritical() << "copyDirRecursively: Failed to copy nonexistent directory" << srcPath;
        return false;
    }
    ...
    {
        qCritical() << "copyDirRecursively: Cannot copy directory" << srcPath << "into itself" << dstPath;
        return false;
    }
    ...
    {
        qCritical() << "copyDirRecursively: Failed to create destination directory" << QFileInfo(dstPath).fileName();
        return false;
    }
    ...
    foreach(const QFileInfo &info, srcDir.entryInfoList(QDir::Dirs | QDir::Files | hiddenFilter | QDir::NoDotAndDotDot))
    {
        ...
        if (info.isDir())
        {
            if (!copyDirRecursively(srcItemPath, dstItemPath, includeHiddenFiles))
            {
                qCritical() << "copyDirRecursively: Failed to copy files from " << srcItemPath << " into " << dstItemPath;
                return false;
            }
        }
        else if (info.isFile())
        ...
        {
            qWarning() << "copyDirRecursively: Unhandled item" << info.filePath();
        }
    }
    return true;
}
...
//-----------------------------------------------------------------------------
bool isDirEmpty(const QDir& directory)
{
...
//------------------------------------------------------------------------------
QString absolutePathFromInternal(const QString &internalPath, const QString &basePath)
{
...
//------------------------------------------------------------------------------
QString internalPathFromAbsolute(const QString &absolutePath, const QString &basePath)
{
...
//------------------------------------------------------------------------------
QModelIndex modelChildIndex(QAbstractItemModel* item, const QModelIndex &parent, int row, int column)
{
...
//------------------------------------------------------------------------------
QModelIndex modelChildIndex(const QAbstractItemModel* item, const QModelIndex &parent, int row, int column)
{
    return modelChildIndex(const_cast<QAbstractItemModel*>(item), parent, row, column);
}
