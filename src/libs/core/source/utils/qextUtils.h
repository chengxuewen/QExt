...
#define QEXT_SOFT_ASSERT(condition) \
    do \
    { \
            if (! (condition) ) \
        { \
...
namespace qext
{
    ...
    QEXT_CORE_API void qListToSTLVector(const QStringList &list, std::vector<char*>& vector);

    ///
    /// \ingroup Core
    /// Convert a QStringList to a Vector of string
    QEXT_CORE_API void qListToSTLVector(const QStringList &list, std::vector<std::string>& vector);

    ///
    /// \ingroup Core
    /// Convert a Vector of string to QStringList
    QEXT_CORE_API void stlVectorToQList(const std::vector<std::string>& vector, QStringList &list);
    ...
    QEXT_CORE_API QSet<QString> qStringListToQSet(const QStringList &list);
    ...
    /// This method was added so that the same code compiles without deprecation warnings
    /// pre and post Qt 5.14.
    QEXT_CORE_API QStringList qSetToQStringList(const QSet<QString>& set);
...
/// pre and post Qt 5.15.
    ...
    QEXT_CORE_API QStringList nameFilterToExtensions(const QString &nameFilter);

    ///
    /// \ingroup Core
    /// Convert a nameFilter to a list of file extensions:
    /// "Images (*.png *.jpg *.tiff)", "Text (*.txt)" -> "*.png", "*.jpg", "*.tiff", "*.txt"
    QEXT_CORE_API QStringList nameFiltersToExtensions(const QStringList &nameFilters);

    ///
    /// \ingroup Core
    /// Convert a wildcar extension filter ("*.jpg") into a regular expression string
    /// "*.jpg" -> ".*\\.jpg?$"
    QEXT_CORE_API QString extensionToRegExp(const QString &extension);
    ...
    /// "*.jpg", "*.txt" -> "(.*\\.jpg?$|.*\\.txt?$)"
    QEXT_CORE_API QRegExp nameFiltersToRegExp(const QStringList &nameFilters);
    ...
    QEXT_CORE_API int significantDecimals(double value, int defaultDecimals = -1);
    ...
    QEXT_CORE_API int orderOfMagnitude(double value);
    ...
    QEXT_CORE_API double closestPowerOfTen(double value);
    ...
    /// \sa QDir::rmdir
    QEXT_CORE_API bool removeDirRecursively(const QString &dirName);
    ...
    /// \return <code>true</code> on success, <code>false</code> otherwise.
    /// \sa QFile::copy
    QEXT_CORE_API bool copyDirRecursively(const QString &srcPath, const QString &dstPath, bool includeHiddenFiles=true);
    ...
    QEXT_CORE_API bool isDirEmpty(const QDir& directory);
    ...
    /// \sa Qt::HANDLE
    QEXT_CORE_API QString qtHandleToString(Qt::HANDLE handle);
    ...
    QEXT_CORE_API qint64 msecsTo(const QDateTime& t1, const QDateTime& t2);
    ...
    QEXT_CORE_API QString absolutePathFromInternal(const QString &internalPath, const QString &basePath);
    ...
    QEXT_CORE_API QString internalPathFromAbsolute(const QString &absolutePath, const QString &basePath);
    ...
    QEXT_CORE_API QTextStream &flush(QTextStream &stream);
    ...
    QEXT_CORE_API QTextStream & endl(QTextStream &stream);
    ...
    /// This method was added so that the same code compiles without deprecation warnings
    /// pre and post Qt 5.8.
    QEXT_CORE_API QModelIndex modelChildIndex(QAbstractItemModel* item, const QModelIndex &parent, int row, int column);
    QEXT_CORE_API QModelIndex modelChildIndex(const QAbstractItemModel* item, const QModelIndex &parent, int row, int column);
    ///}@
...
//------------------------------------------------------------------------------
    ...
    bool removeOne(std::list<T>& list, const T& value)
    {
        typename std::list<T>::iterator it = std::find(list.begin(), list.end(), value);
        if (it != list.end())
        {
            list.erase(it);
            return true;
