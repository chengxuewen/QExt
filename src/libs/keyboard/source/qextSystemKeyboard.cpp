...
bool OpenOskKeyboard()
{
    ...
    {
        qCWarning(lcQExtKeyboard) << "Wow64DisableWow64FsRedirection fail err code:" << GetLastError();
        return false;
    }
}
...
bool IsNewTabTipKeyboardVisable()
{
    ...
    if (!parent)
    {
        qCWarning(lcQExtKeyboard) << "no more windows, keyboard state is unknown. class:"
    ...
    if (!wnd)
    {
        qCWarning(lcQExtKeyboard) << "it's a child of a WindowParentClass1709 window - the keyboard is open. class:"
    ...
    {
        qCWarning(lcQExtKeyboard) << "DwmGetWindowAttribute err:" << GetLastError();
        return false;
    }
    qCWarning(lcQExtKeyboard) << "DwmGetWindowAttribute cloaked=" << cloaked;
    return 0 == cloaked;
}
...
bool IsOldTabTipKeyboardVisable()
{
    ...
    if (!touchhWnd)
    {
        qCWarning(lcQExtKeyboard) << "without window:" << UnicodeToUtf8(kKeyboardWindowClass);
        return false;
    }

    unsigned long style = GetWindowLong(touchhWnd, GWL_STYLE);
    // 由于有的系统在键盘不显示时候只是多返回一个WS_DISABLED这个字段。所以加一个它的判断
    qCWarning(lcQExtKeyboard) << "WS_CLIPSIBLINGS:" << (style & WS_CLIPSIBLINGS)
...
bool OpenTabTipByProcess(const std::string &tabTipPath)
{
    ...
    {
        qCWarning(lcQExtKeyboard) << "ShellExecuteW tabTip err:" << GetLastError();
        return false;
    }
    qCWarning(lcQExtKeyboard) << "ShellExecuteW tabTip success";
    return true;
}
...
bool OpenTabTip()
{
    ...
    // qCWarning(lcQExtKeyboard) << "os version:" << version.ToString();
    // if (IsNewTabTipKeyboardVisable() || IsOldTabTipKeyboardVisable())
    // {
    //     // qCWarning(lcQExtKeyboard) << "keyboard is visible";
    //     return true;
    // }
