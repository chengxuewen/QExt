...
void QEXTFramelessHelper::setTitleBarHeight(int value)
{
    ...
    if (value != d->m_titleBarHeight)
    {
        ...
        emit this->titleBarHeightChanged(value);
    }
}
...
bool QEXTFramelessHelper::eventFilter(QObject *obj, QEvent *event)
{
    Q_D(QEXTFramelessHelper);
    if (event->type() == QEvent::WindowStateChange)
    {
        ...
        if (maximized != d->m_maximized)
        {
            ...
            emit this->maximizedChanged(maximized);
        }
    }
    else if (event->type() == QEvent::WinIdChange)
    {
        if (QEXT_DECL_NULLPTR == d->m_helper)
        {
            QWindow *w = d->m_window->windowHandle();

            d->m_helper = new QEXTNativeWindowHelper(w, d);
            connect(d->m_helper, &QEXTNativeWindowHelper::scaleFactorChanged, this, &QEXTFramelessHelper::scaleFactorChanged);
            if (!qFuzzyCompare(d->m_helper->scaleFactor(), 1.0))
            {
                emit this->scaleFactorChanged(d->m_helper->scaleFactor());
            }
        }
    }

#if QT_VERSION < QT_VERSION_CHECK(5, 9, 0) && defined(QEXT_OS_WIN32)
