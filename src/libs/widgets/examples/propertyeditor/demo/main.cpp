...
int main(int argc, char **argv)
{
    ...
    QExtPEBoolPropertyManager *boolManager = new QExtPEBoolPropertyManager(w);
    QExtPEIntPropertyManager *intManager = new QExtPEIntPropertyManager(w);
    QExtPEStringPropertyManager *stringManager = new QExtPEStringPropertyManager(w);
    QExtPESizePropertyManager *sizeManager = new QExtPESizePropertyManager(w);
    QExtPERectPropertyManager *rectManager = new QExtPERectPropertyManager(w);
    QExtPESizePolicyPropertyManager *sizePolicyManager = new QExtPESizePolicyPropertyManager(w);
    QExtPEEnumPropertyManager *enumManager = new QExtPEEnumPropertyManager(w);
    QExtPEGroupPropertyManager *groupManager = new QExtPEGroupPropertyManager(w);

    QExtPEProperty *item0 = groupManager->addProperty("QObject");

    QExtPEProperty *item1 = stringManager->addProperty("objectName");
    item0->addSubProperty(item1);

    QExtPEProperty *item2 = boolManager->addProperty("enabled");
    item0->addSubProperty(item2);

    QExtPEProperty *item3 = rectManager->addProperty("geometry");
    item0->addSubProperty(item3);

    QExtPEProperty *item4 = sizePolicyManager->addProperty("sizePolicy");
    item0->addSubProperty(item4);

    QExtPEProperty *item5 = sizeManager->addProperty("sizeIncrement");
    item0->addSubProperty(item5);

    QExtPEProperty *item7 = boolManager->addProperty("mouseTracking");
    item0->addSubProperty(item7);

    QExtPEProperty *item8 = enumManager->addProperty("direction");
    ...
    QExtPEProperty *item9 = intManager->addProperty("value");
    intManager->setRange(item9, -100, 100);
    item0->addSubProperty(item9);

    QExtPECheckBoxFactory *checkBoxFactory = new QExtPECheckBoxFactory(w);
    QExtPESpinBoxFactory *spinBoxFactory = new QExtPESpinBoxFactory(w);
    QExtPESliderFactory *sliderFactory = new QExtPESliderFactory(w);
    QExtPEScrollBarFactory *scrollBarFactory = new QExtPEScrollBarFactory(w);
    QExtPELineEditFactory *lineEditFactory = new QExtPELineEditFactory(w);
    QExtPEEnumEditorFactory *comboBoxFactory = new QExtPEEnumEditorFactory(w);

    QExtPEAbstractPropertyEditor *editor1 = new QExtPEPropertyTreeEditor();
    ...
    QExtPEAbstractPropertyEditor *editor2 = new QExtPEPropertyTreeEditor();
    editor2->addProperty(item0);

    QExtPEAbstractPropertyEditor *editor3 = new QExtPEPropertyGroupBoxEditor();
    ...
    QExtPEAbstractPropertyEditor *editor4 = new QExtPEPropertyGroupBoxEditor();
    ...
    QExtPEAbstractPropertyEditor *editor5 = new QExtPEPropertyButtonEditor();
