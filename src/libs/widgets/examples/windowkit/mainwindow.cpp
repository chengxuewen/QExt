...
{
public:
    explicit ClockWidget(QWidget *parent = QEXT_NULLPTR) : QLabel(parent)
    {
        startTimer(100);
        setAlignment(Qt::AlignCenter);
    }

    ~ClockWidget() QEXT_OVERRIDE = default;

protected:
    void timerEvent(QTimerEvent *event) QEXT_OVERRIDE
    {
...
void MainWindow::installWindowAgent()
{
    ...
    windowAgent = new WidgetWindowAgent(this);
...
#ifdef Q_OS_MAC
    struct CallbackInterface : public QExtScreenRectCallbackInterface
    {
        QRect screenRect(const QSize &size)
        {
            static constexpr const int width = 75;
            return QRect(QPoint(size.width() - width, 0), QSize(width, size.height())); //
        }
    };
    windowAgent->setSystemButtonAreaCallback(new CallbackInterface);
#endif
