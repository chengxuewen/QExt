...
void QTKDeviceWatcherPrivate::parseLine(const QByteArray &line)
{
...
#endif //USE_REGEXP
    QTKDeviceChangeEvent *event = 0;

    if (action_str==QLatin1String("add")) {
        emitDeviceAdded(dev);
        event = new QTKDeviceChangeEvent(QTKDeviceChangeEvent::Add, dev);
    } else if (action_str==QLatin1String("remove")) {
        emitDeviceRemoved(dev);
        event = new QTKDeviceChangeEvent(QTKDeviceChangeEvent::Remove, dev);
    } else if (action_str==QLatin1String("change")) {
        emitDeviceChanged(dev);
        event = new QTKDeviceChangeEvent(QTKDeviceChangeEvent::Change, dev);
    }
