...
#ifndef QTKSERVICEREFERENCE_H
#define QTKSERVICEREFERENCE_H
...
class QTKServiceReferencePrivate;
class QTKServiceEvent;

/**
 * @ingroup QTKPluginFramework
 * @brief The QTKServiceReference class
 ...
 */
class QTK_PLUGINFRAMEWORK_EXPORT QTKServiceReference
{
public:
    /**
     * @brief QTKServiceReference
     * Creates an invalid ctkServiceReference object. You can use
     * this object in boolean expressions and it will evaluate to
     * <code>false</code>.
     */
    QTKServiceReference();
    QTKServiceReference(const QTKServiceReference &serviceRef);
    ~QTKServiceReference();
     ...
     *         zero.
     */
    QList<QSharedPointer<QTKPlugin>> getUsingPlugins() const;
     ...
     */
    QTKServiceReference& operator=(int iNull);
     ...
     * service id.
     *
     * @param serviceRef The <code>ctkServiceReference</code> to be compared.
     ...
     */
    bool operator<(const QTKServiceReference &serviceRef) const;
    bool operator==(const QTKServiceReference &serviceRef) const;
    QTKServiceReference& operator=(const QTKServiceReference &serviceRef);

protected:
    QTKServiceReference(QTKServiceRegistrationPrivate *pReg);
    friend uint QTK_PLUGINFRAMEWORK_EXPORT qHash(const QTKServiceReference &serviceRef);

    friend class QTKLDAPSearchFilter;
    friend class QTKServiceRegistrationPrivate;
    friend class QTKPluginContext;
    friend class QTKPluginPrivate;
    friend class QTKPluginFrameworkListeners;
    template<class S, class T> friend class QTKServiceTracker;
    template<class S, class T> friend class QTKServiceTrackerPrivate;
    template<class S, class R, class T> friend class QTKPluginAbstractTracked;

    QTKServiceReferencePrivate *d_ptr;

private:
    Q_DECLARE_PRIVATE(QTKServiceReference)
};
 ...
 * @{
 */
uint QTK_PLUGINFRAMEWORK_EXPORT qHash(const QTKServiceReference &serviceRef);
QDebug QTK_PLUGINFRAMEWORK_EXPORT operator<<(QDebug dbg, const QTKServiceReference &serviceRef);
/** @}*/

QTK_PLUGINFRAMEWORK_END_NAMESPACE

Q_DECLARE_METATYPE(QTK_PLUGINFRAMEWORK_NAMESPACE::QTKServiceReference)

#endif // QTKSERVICEREFERENCE_H
