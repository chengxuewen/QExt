 ...
 *
 * <p>
 * A <code>%QExtOsgiPlugin</code> object is the access point to define the lifecycle of
 * an installed plugin. Each plugin installed in the plugin environment must have
 * an associated <code>%QExtOsgiPlugin</code> object.
 *
 * <p>
 ...
 * <code>%QExtOsgiPlugin</code> objects, and these objects are only valid within the
 * Framework that created them.
 *
 * @remarks This class is thread safe.
 */
class QEXT_OSGI_API QExtOsgiPlugin {
...
public:
     ...
     * <code>%QExtOsgiPlugin</code> object may still be available and used for
     * introspection.
     */
     ...
     * {@link QExtOsgiPluginConstants::REQUIRE_PLUGIN} Manifest header.
     * </ul>
     * <p>
     ...
     * state when the plugin's {@link QExtOsgiPluginActivator::start} method is called. If the
     * <code>QExtOsgiPluginActivator::start</code> method completes without exception,
     * then the plugin has successfully started and must move to the
     * <code>ACTIVE</code> state.
     * <p>
     * If the plugin does not have a
     * {@link QExtOsgiPluginConstants#ACTIVATION_EAGER eager activation policy}, then the
     ...
     * when the plugin's {@link QExtOsgiPluginActivator::stop} method is called. When the
     * <code>QExtOsgiPluginActivator::stop</code> method completes the plugin is
   ...
   */
     ...
     * plugin's declared
     * {@link QExtOsgiPluginConstants#PLUGIN_ACTIVATIONPOLICY activation policy}.
     *
     * <p>
     ...
     * activation policy.
     *
     * @see QExtOsgiPluginConstants#PLUGIN_ACTIVATIONPOLICY
     * @see #start(const StartOptions&)
     */
   ...
   * <code>QExtIllegalStateException</code> is thrown.
   * <p>
   ...
   * <code>QExtOsgiPluginException</code> is thrown to indicate this plugin was unable
   * to be started.
   *
   ...
   * a <code>QExtOsgiPluginException</code> is thrown.
   *
   ...
   * <li>A plugin event of type {@link QExtOsgiPluginEvent::LAZY_ACTIVATION} is fired.
   ...
   * plugin's declared activation policy is {@link QExtOsgiPluginConstants#ACTIVATION_EAGER
   * eager} then:
   * <i></i>
   * <li>This plugin's state is set to <code>STARTING</code>.
   *
   * <li>A plugin event of type {@link QExtOsgiPluginEvent::STARTING} is fired.
   *
   * <li>The {@link QExtOsgiPluginActivator::start} method of this plugin's
   * <code>QExtOsgiPluginActivator</code>, is called. If the
   * <code>QExtOsgiPluginActivator</code> throws an exception then:
   * <ul>
   * <li>This plugin's state is set to <code>STOPPING</code>.
   * <li>A plugin event of type {@link QExtOsgiPluginEvent::STOPPING} is fired.
   ...
   * <li>A plugin event of type {@link QExtOsgiPluginEvent::STOPPED} is fired.
   * <li>A <code>QExtOsgiPluginException</code> is then thrown.
   * </ul>
   * <i></i>
   * <li>If this plugin's state is <code>UNINSTALLED</code>, because this
   * plugin was uninstalled while the <code>QExtOsgiPluginActivator::start</code>
   * method was running, a <code>QExtOsgiPluginException</code> is thrown.
   *
   * <li>This plugin's state is set to <code>ACTIVE</code>.
   *
   * <li>A plugin event of type {@link QExtOsgiPluginEvent::STARTED} is fired.
   * </ol>
   *
   * <b>Preconditions </b>
   * <ul>
   ...
   * <li><code>QExtOsgiPluginActivator::start()</code> has been called and did not
   ...
   * @throws QExtOsgiPluginException If this plugin could not be started. This could
   *         be because a code dependency could not be resolved or the
   *         <code>QExtOsgiPluginActivator</code> could not be loaded or
   *         threw an exception.
   * @throws QExtIllegalStateException If this plugin has been uninstalled or this
   ...
   * state to <code>STOPPING</code>, the {@link QExtOsgiPluginActivator#stop} method
   * of this plugin's <code>QExtOsgiPluginActivator</code> is
   * called. If that method throws an exception, this method must continue to
   * stop this plugin and a <code>QExtOsgiPluginException</code> must be thrown after
   ...
   *
   * <li>If this plugin's state is <code>UNINSTALLED</code>, because this
   * plugin was uninstalled while the <code>QExtOsgiPluginActivator::stop</code> method
   * was running, a <code>QExtOsgiPluginException</code> must be thrown.
   *
   * <li>This plugin's state is set to <code>RESOLVED</code>.
   *
   * <li>A plugin event of type {@link QExtOsgiPluginEvent::STOPPED} is fired.
   ...
   * <code>STOPPING</code> &#125;.
   * <li><code>QExtOsgiPluginActivator::stop</code> has been called and did not throw
   * an exception.
   * </ul>
   * <b>Postconditions, when an exception is thrown </b>
   * <ul>
   ...
   *        {@link #STOP_TRANSIENT}.
   * @throws QExtOsgiPluginException If this plugin's <code>QExtOsgiPluginActivator</code>
   *         threw an exception.
   * @throws QExtIllegalStateException If this plugin has been uninstalled or this
   *         plugin tries to change its own state.
   */
   ...
   * this plugin's {@link QExtOsgiPluginConstants#PLUGIN_UPDATELOCATION
   ...
   * <code>QExtOsgiPlugin::stop()</code> method. If <code>QExtOsgiPlugin::stop()</code> throws an
   ...
   * <code>QExtOsgiPluginException</code> is thrown after completion of the
   * remaining steps.
   *
   ...
   * plugin event of type {@link QExtOsgiPluginEvent#UPDATED} is fired.
   *
   * <li>If this plugin's state was originally <code>ACTIVE</code>, the
   * updated plugin is started as described in the <code>QExtOsgiPlugin::start()</code>
   * method. If <code>QExtOsgiPlugin::start()</code> throws an exception, a Framework
   * event of type {@link QExtOsgiPluginFrameworkEvent#PLUGIN_ERROR} is fired containing the
   * exception.
   * </ol>
   *
   * <b>Preconditions </b>
   ...
   *        {@link QExtOsgiPluginConstants#PLUGIN_UPDATELOCATION Plugin-UpdateLocation}
   *        Manifest header, if present, or this plugin's original location.
   * @throws QExtOsgiPluginException If the update location cannot be read or the update
   *         fails.
   * @throws QExtIllegalStateException If this plugin has been uninstalled or this
   *         plugin tries to change its own state.
   * @see #stop()
   ...
   * or <code>STOPPING</code>, this plugin is stopped as described in the
   * <code>QExtOsgiPlugin::stop</code> method. If <code>QExtOsgiPlugin::stop</code> throws an
   * exception, a Framework event of type QExtOsgiPluginFrameworkEvent::PLUGIN_ERROR is
   * fired containing the exception.
   *
   * <li>This plugin's state is set to <code>UNINSTALLED</code>.
   *
   * <li>A plugin event of type QExtOsgiPluginEvent::UNINSTALLED is fired.
   *
   ...
   * </ul>
   *
   * @throws QExtOsgiPluginException If the uninstall failed. This can occur if
   ...
   */
    ...
    /**
   * Returns this plugin's {@link QExtOsgiPluginContext}. The returned
   * <code>QExtOsgiPluginContext</code> can be used by the caller to act on behalf
   * of this plugin.
   *
   * <p>
   * If this plugin is not in the {@link #STARTING}, {@link #ACTIVE}, or
   * {@link #STOPPING} states, then this
   * plugin has no valid <code>QExtOsgiPluginContext</code>. This method will
   * return <code>0</code> if this plugin has no valid
   * <code>QExtOsgiPluginContext</code>.
   *
   * @return A <code>QExtOsgiPluginContext</code> for this plugin or
   *         <code>0</code> if this plugin has no valid
   *         <code>QExtOsgiPluginContext</code>.
   */
    QExtOsgiPluginContext* getPluginContext() const;
   ...
   * <code>QExtOsgiPluginContext::installPlugin</code> when a plugin is installed.
   ...
   *
   * You can use the returned <code>QExtOsgiPluginLocalization</code>
   ...
   *        <code>QExtOsgiPluginLocalization</code> object.
   * @param base The base name of the .qm message file which contains
   *        translated messages. Defaults to
   *        <code>QExtOsgiPluginConstants::PLUGIN_LOCALIZATION_DEFAULT_BASENAME</code>.
   ...
   * specified version then {@link QExtOsgiVersion#emptyVersion} is returned.
   *
   * <p>
   ...
   */
    QExtOsgiVersion getVersion() const;

protected:

    friend class QExtOsgiPluginFramework;
    friend class QExtOsgiPluginFrameworkLauncherPrivate;
    friend class QExtOsgiPluginFrameworkPrivate;
    friend class QExtOsgiPluginFrameworkContext;
    friend class QExtOsgiPlugins;
...
private:
    Q_DECLARE_PRIVATE(QExtOsgiPlugin)
    Q_DISABLE_COPY(QExtOsgiPlugin)
};
...
Q_DECLARE_METATYPE(QExtOsgiPlugin*)
Q_DECLARE_METATYPE(QSharedPointer<QExtOsgiPlugin>)

Q_DECLARE_OPERATORS_FOR_FLAGS(QExtOsgiPlugin::States)
Q_DECLARE_OPERATORS_FOR_FLAGS(QExtOsgiPlugin::StartOptions)
Q_DECLARE_OPERATORS_FOR_FLAGS(QExtOsgiPlugin::StopOptions)

QEXT_OSGI_API QDebug operator<<(QDebug debug, QExtOsgiPlugin::State state);
QEXT_OSGI_API QDebug operator<<(QDebug debug, const QExtOsgiPlugin& plugin);
QEXT_OSGI_API QDebug operator<<(QDebug debug, QExtOsgiPlugin const * plugin);

QEXT_OSGI_API QExtOsgiLogStream& operator<<(QExtOsgiLogStream& stream, QExtOsgiPlugin const * plugin);
QEXT_OSGI_API QExtOsgiLogStream& operator<<(QExtOsgiLogStream& stream, const QSharedPointer<QExtOsgiPlugin>& plugin);

/** @}*/

#endif // _QEXTOSGIPLUGIN_H
