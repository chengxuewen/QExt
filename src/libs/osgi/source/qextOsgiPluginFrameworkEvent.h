...
#ifndef _QEXTOSGIPLUGINFRAMEWORKEVENT_H
#define _QEXTOSGIPLUGINFRAMEWORKEVENT_H
...
class QExtOsgiPluginFrameworkEventData;
 ...
 *
 * <p>
 * <code>QExtOsgiPluginFrameworkEvent</code> objects are delivered to slots connected
 * via QExtOsgiPluginContext::connectFrameworkListener when a general event occurs
 * within the plugin environment.
 * A type code is used to identify the event type for future extendibility.
 *
 * @see QExtOsgiPluginContext#connectFrameworkListener
 * @see QExtOsgiEventBus
 */
class QEXT_OSGI_API QExtOsgiPluginFrameworkEvent
{

    QSharedDataPointer<QExtOsgiPluginFrameworkEventData> d;
...
public:
   ...
   */
    QExtOsgiPluginFrameworkEvent();

    ~QExtOsgiPluginFrameworkEvent();

    /**
   * Can be used to check if this QExtOsgiPluginFrameworkEvent instance is valid,
   ...
   */
    QExtOsgiPluginFrameworkEvent(Type type, QSharedPointer<QExtOsgiPlugin> plugin, const QExtException& fwException);
...
Q_DECLARE_METATYPE(QExtOsgiPluginFrameworkEvent);
