...
#ifndef _QEXTOSGILDAPEXPR_P_H
#define _QEXTOSGILDAPEXPR_P_H
...
public:
  ...
  /**
   * Creates an invalid QExtOsgiLDAPExpr object. Use with care.
   *
   * @see isNull()
   */
  QExtOsgiLDAPExpr();
  ...
  //!
  QExtOsgiLDAPExpr(const QString &filter);

  //!
  QExtOsgiLDAPExpr(const QExtOsgiLDAPExpr& other);

  QExtOsgiLDAPExpr& operator=(const QExtOsgiLDAPExpr& other);
   ...
   * constructed using QExtOsgiLDAPExpr().
   *
  ...
  //!
  static bool query(const QString &filter, const QExtOsgiDictionary &pd);

  //! Evaluate this LDAP filter.
  bool evaluate(const QExtOsgiServiceProperties &p, bool matchCase) const;
...
private:
  ...
  //!
  QExtOsgiLDAPExpr(int op, const QList<QExtOsgiLDAPExpr> &args);

  //!
  QExtOsgiLDAPExpr(int op, const QString &attrName, const QString &attrValue);

  //!
  static QExtOsgiLDAPExpr parseExpr(ParseState &ps);

  //!
  static QExtOsgiLDAPExpr parseSimple(ParseState &ps);
  ...
  //! Shared pointer
  QSharedDataPointer<QExtOsgiLDAPExprData> d;

};


#endif // _QEXTOSGILDAPEXPR_P_H
