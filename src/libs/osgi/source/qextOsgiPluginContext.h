...
#ifndef _QEXTOSGIPLUGINCONTEXT_H_
#define _QEXTOSGIPLUGINCONTEXT_H_
...
class QExtOsgiPluginContextPrivate;
 ...
 *
 * <p>
 * <code>QExtOsgiPluginContext</code> methods allow a plugin to:
 * <ul>
 ...
 * <li>Get the {@link QExtOsgiPlugin} object for a plugin.
 * <li>Create <code>QFile</code> objects for files in a persistent storage
 * area provided for the plugin by the Framework.
 * </ul>
 *
 * <p>
 * A <code>QExtOsgiPluginContext</code> object will be created and provided to the
 * plugin associated with this context when it is started using the
 * {@link QExtOsgiPluginActivator::start} method. The same <code>QExtOsgiPluginContext</code>
 * object will be passed to the plugin associated with this context when it is
 * stopped using the {@link QExtOsgiPluginActivator::stop} method. A
 * <code>QExtOsgiPluginContext</code> object is generally for the private use of its
 * associated plugin and is not meant to be shared with other plugins in the
 * plugin environment.
 *
 * <p>
 * The <code>QExtOsgiPlugin</code> object associated with a <code>QExtOsgiPluginContext</code>
 * object is called the <em>context plugin</em>.
 *
 * <p>
 * The <code>QExtOsgiPluginContext</code> object is only valid during the execution of
 ...
 * <code>ACTIVE</code> plugin states. If the <code>QExtOsgiPluginContext</code>
 * object is used subsequently, a <code>QExtIllegalStateException</code> must be
 * thrown. The <code>QExtOsgiPluginContext</code> object must never be reused after
 * its context plugin is stopped.
 *
 * <p>
 * The Framework is the only entity that can create <code>QExtOsgiPluginContext</code>
 * objects and they are only valid within the Framework that created them.
 *
 * @remarks This class is thread safe.
 */
class QEXT_OSGI_API QExtOsgiPluginContext
{

public:

    ~QExtOsgiPluginContext();
   ...
   * <code>QExtOsgiPluginContext</code>. This plugin is called the context plugin.
   *
   * @return The <code>QExtOsgiPlugin</code> object associated with this
   *         <code>QExtOsgiPluginContext</code>.
   * @throws QExtIllegalStateException If this QExtOsgiPluginContext is no
   *         longer valid.
   */
   ...
   * @return A <code>QExtOsgiPlugin</code> object or <code>0</code> if the
   *         identifier does not match any installed plugin.
   */
    QSharedPointer<QExtOsgiPlugin> getPlugin(long id) const;
   ...
   *
   * @return A QList of <code>QExtOsgiPlugin</code> objects, one object per
   *         installed plugin.
   */
   ...
   * <code>QExtOsgiServiceRegistration</code> object is returned. The
   * <code>QExtOsgiServiceRegistration</code> object is for the private use of the
   ...
   * {@link QExtOsgiServiceFactory} interface to have more flexibility in providing
   * service objects to other plugins.
   *
   * <p>
   * The following steps are required to register a service:
   * <ol>
   * <li>If <code>service</code> is not a <code>QExtOsgiServiceFactory</code>, an
   * <code>QExtInvalidArgumentException</code> is thrown if <code>service</code>
   * is not an instance of all the specified class names.
   * <li>The Framework adds the following service properties to the service
   * properties from the specified <code>QExtOsgiDictionary</code> (which may be
   * omitted): <br/>
   * A property named {@link QExtOsgiPluginConstants#SERVICE_ID} identifying the
   * registration number of the service <br/>
   * A property named {@link QExtOsgiPluginConstants#OBJECTCLASS} containing all the
   * specified classes. <br/>
   * Properties with these names in the specified <code>QExtOsgiDictionary</code> will
   * be ignored.
   * <li>The service is added to the Framework service registry and may now be
   * used by other plugins.
   * <li>A service event of type {@link QExtOsgiServiceEvent#REGISTERED} is fired.
   * <li>A <code>QExtOsgiServiceRegistration</code> object for this registration is
   * returned.
   * </ol>
   *
   * @param clazzes The class names under which the service can be located.
   *        The class names will be stored in the service's
   *        properties under the key {@link QExtOsgiPluginConstants#OBJECTCLASS}.
   * @param service The service object or a <code>QExtOsgiServiceFactory</code>
   *        object.
   * @param properties The properties for this service. The keys in the
   *        properties object must all be <code>QString</code> objects. See
   *        {@link QExtOsgiPluginConstants} for a list of standard service property keys.
   *        Changes should not be made to this object after calling this
   *        method. To update the service's properties the
   *        {@link QExtOsgiServiceRegistration::setProperties} method must be called.
   *        The set of properties may be omitted if the service has
   *        no properties.
   * @return A <code>QExtOsgiServiceRegistration</code> object for use by the plugin
   *         registering the service to update the service's properties or to
   *         unregister the service.
   * @throws QExtInvalidArgumentException If one of the following is true:
   *         <ul>
   *         <li><code>service</code> is <code>0</code>. <li><code>service
   *         </code> is not a <code>QExtOsgiServiceFactory</code> object and is not an
   ...
   * registerService(const QStringList&, QObject*, const QExtOsgiDictionary&) and is provided as
   ...
   * QExtOsgiPluginConstants::OBJECTCLASS property will be a QStringList, rather
   * than just a single string.
   *
   * @param clazz The class name under which the service can be located.
   * @param service The service object or a QExtOsgiServiceFactory object.
   * @param properties The properties for this service.
   * @return A QExtOsgiServiceRegistration object for use by the plugin
   *         registering the service to update the service's properties or to
   *         unregister the service.
   * @throws QExtIllegalStateException If this QExtOsgiPluginContext is no longer valid.
   ...
   * which satisfy the filter expression. See {@link QExtOsgiLDAPSearchFilter} for a description
   ...
   * an <code>QExtInvalidArgumentException</code> will be thrown with a human readable
   * message where the filter became unparsable.
   *
   * <p>
   * The result is a list of <code>QExtOsgiServiceReference</code> objects for all
   ...
   * {@link QExtOsgiPluginConstants::OBJECTCLASS objectClass} property.
   ...
   *        services.
   * @return A list of <code>QExtOsgiServiceReference</code> objects or
   *         an empty list if no services are registered which satisfy the
   *         search.
   * @throws QExtInvalidArgumentException If the specified <code>filter</code>
   ...
   *
   * <p>
   * The returned <code>QExtOsgiServiceReference</code> object is valid at the time of
   * the call to this method. However as the Framework is a very dynamic
   * environment, services can be modified or unregistered at any time.
   *
   * <p>
   * This method is the same as calling
   * {@link QExtOsgiPluginContext::getServiceReferences(const QString&, const QString&)} with an
   * empty filter expression. It is provided as a convenience for
   * when the caller is interested in any service that implements the
   * specified class.
   * <p>
   * If multiple such services exist, the service with the highest ranking (as
   * specified in its {@link QExtOsgiPluginConstants::SERVICE_RANKING} property) is returned.
   * <p>
   * If there is a tie in ranking, the service with the lowest service ID (as
   * specified in its {@link QExtOsgiPluginConstants::SERVICE_ID} property); that is, the
   * service that was registered first is returned.
   *
   * @param clazz The class name with which the service was registered.
   * @return A <code>QExtOsgiServiceReference</code> object, or <code>0</code> if
   ...
   * {@link #getService(const QExtOsgiServiceReference&)} the context plugin's use count for
   * that service is incremented by one. Each time the service is released by
   * {@link #ungetService(const QExtOsgiServiceReference&)} the context plugin's use count
   ...
   * <code>QExtOsgiServiceFactory</code> interface, the
   * {@link QExtOsgiServiceFactory::getService} method is
   ...
   * service object. <br>
   * If the service object returned by the <code>QExtOsgiServiceFactory</code> object
   * is not an instance of all the classes named when the service
   * was registered or the <code>QExtOsgiServiceFactory</code> object throws an
   * exception, <code>0</code> is returned and a Framework event of type
   * {@link QExtOsgiPluginFrameworkEvent::PLUGIN_ERROR} containing a {@link QExtOsgiServiceException}
   ...
   *         <code>QExtOsgiServiceFactory</code> does not implement the classes under
   *         which it was registered or the <code>QExtOsgiServiceFactory</code> threw
   *         an exception.
   * @throws QExtIllegalStateException If this QExtOsgiPluginContext is no
   *         longer valid.
   * @throws QExtInvalidArgumentException If the specified
   *         <code>QExtOsgiServiceReference</code> was not created by the same
   *         framework instance as this <code>QExtOsgiPluginContext</code> or
   ...
   *         registered, the service object returned by a
   *         <code>QExtOsgiServiceFactory</code> does not implement the classes under
   *         which it was registered, the <code>QExtOsgiServiceFactory</code> threw
   ...
   *         framework instance as this <code>QExtOsgiPluginContext</code> or
   *         if it is invalid (default constructed).
   * @see #getService(const QExtOsgiServiceReference&)
   * @see #ungetService(const QExtOsgiServiceReference&)
   * @see QExtOsgiServiceFactory
   */
    template<class S>
   ...
   * <code>QExtOsgiServiceReference</code> object. If the context plugin's use count
   ...
   * and the service was registered with a <code>QExtOsgiServiceFactory</code> object,
   * the
   * {@link QExtOsgiServiceFactory#ungetService}
   ...
   * @throws QExtInvalidArgumentException If the specified
   *         <code>QExtOsgiServiceReference</code> was not created by the same
   *         framework instance as this <code>QExtOsgiPluginContext</code>.
   * @see #getService
   * @see QExtOsgiServiceFactory
   */
   ...
   * ensure that the plugin has the <code>qextFilePermission</code> with
   ...
   * installed, the <code>QExtOsgiPlugin</code> object for that plugin is returned.
   *
   * <li>The plugin's content is read from the input stream. If this fails, a
   * {@link QExtOsgiPluginException} is thrown.
   *
   * <li>The plugin's associated resources are allocated. The associated
   * resources minimally consist of a unique identifier and a persistent
   * storage area. If this step fails, a <code>QExtOsgiPluginException</code>
   * is thrown.
   *
   * <li>The plugin's state is set to <code>INSTALLED</code>.
   *
   * <li>A plugin event of type {@link QExtOsgiPluginEvent#INSTALLED} is fired.
   *
   * <li>The <code>QExtOsgiPlugin</code> object for the newly or previously installed
   * plugin is returned.
   * </ol>
   *
   ...
   * @return The <code>QExtOsgiPlugin</code> object of the installed plugin.
   * @throws QExtOsgiPluginException If the I/O device cannot be read or the
   *         installation failed.
   * @throws QExtIllegalStateException If this QExtOsgiPluginContext is no longer valid.
   */
   ...
   * must be "slotName(QExtOsgiPluginEvent)".
   *
   ...
   * of the slot must be "slotName(QExtOsgiPluginFrameworkEvent)".
   *
   * @param receiver The object to connect to.
   * @param slot The slot to be connected.
   ...
   * of the slot must be "slotName(const QExtOsgiServiceEvent&)", but only the name
   * of the slot must be provided as the argument.
   * See {@link QExtOsgiLDAPSearchFilter} for a description of
   * the filter syntax.
   *
   * <p>
   ...
   * {@link QExtOsgiPluginConstants#OBJECTCLASS} property. If <code>filter</code> is
   * <code>null</code>, all services are considered to match the filter.
   *
   * <p>
   * When using a <code>filter</code>, it is possible that the
   * <code>QExtOsgiServiceEvent</code>s for the complete lifecycle of a service
   ...
   * @throws QExtInvalidArgumentException If <code>filter</code> contains an
   *         invalid filter string that cannot be parsed.
   * @throws QExtIllegalStateException If this QExtOsgiPluginContext is no
   *         longer valid.
   * @see QExtOsgiServiceEvent
...
protected:
    ...
    friend class QExtOsgiPluginFrameworkPrivate;
...
private:
    Q_DECLARE_PRIVATE(QExtOsgiPluginContext)
};


#endif // _QEXTOSGIPLUGINCONTEXT_H_
