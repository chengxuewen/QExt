...
//----------------------------------------------------------------------------
QString QExtOsgiPluginFrameworkUtil::getFrameworkDir(QExtOsgiPluginFrameworkContext* /*ctx*/)
{
    QExtOsgiLocation* location = QExtOsgiLocationManager::getConfigurationLocation();
    if (location)
    {
                                                 ...
                                                 const QString& name)
{
    ...
    if (fwdir.isEmpty())
    {
        throw QExtRuntimeException("The framework storage directory is empty");
    }
    QDir dir(fwdir + "/" + name);
    if (dir.exists())
    {
        if (!QFileInfo(dir.absolutePath()).isDir())
        {
            ...
            throw QExtRuntimeException(msg);
        }
    }
    else
    {
...
bool pluginIdLessThan(const QSharedPointer<QExtOsgiPlugin>& p1, const QSharedPointer<QExtOsgiPlugin>& p2)
{
