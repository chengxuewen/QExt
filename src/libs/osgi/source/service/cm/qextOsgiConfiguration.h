...
#ifndef _QEXTOSGICONFIGURATION_H
#define _QEXTOSGICONFIGURATION_H
...
/**
 * \ingroup ConfigAdmin
 * The configuration information for a <code>QExtOsgiManagedService</code> or
 * <code>QExtOsgiManagedServiceFactory</code> object.
 *
 * The Configuration Admin service uses this interface to represent the
 * configuration information for a <code>QExtOsgiManagedService</code> or for a
 * service instance of a <code>QExtOsgiManagedServiceFactory</code>.
 *
 * <p>
 * A <code>QExtOsgiConfiguration</code> object contains a configuration dictionary and
 ...
 * register a <code>QExtOsgiManagedService</code> or
 * <code>QExtOsgiManagedServiceFactory</code>. Only administrative plugins, and
 ...
 * <code>QExtOsgiPlugin#getLocation()</code>). The purpose of binding a
 * <code>QExtOsgiConfiguration</code> object to a location is to make it impossible
 ...
 * location registers a corresponding <code>QExtOsgiManagedService</code> object or
 * <code>QExtOsgiManagedServiceFactory</code> object, then the configuration is not
 ...
 * registers a <code>QExtOsgiManagedService</code> or
 * <code>QExtOsgiManagedServiceFactory</code> object with the corresponding PID.
 * <p>
 * The same <code>QExtOsgiConfiguration</code> object is used for configuring both a
 ...
 */
struct QEXT_OSGI_API QExtOsgiConfiguration
{

    virtual ~QExtOsgiConfiguration();

    /**
   * Get the PID for this <code>QExtOsgiConfiguration</code> object.
   *
   * @return the PID for this <code>QExtOsgiConfiguration</code> object.
   * @throws QExtIllegalStateException if this configuration has been deleted
   */
    virtual QString getPid() const = 0;

    /**
   * Return the properties of this <code>QExtOsgiConfiguration</code> object.
   *
   ...
   */
    virtual QExtOsgiDictionary getProperties() const = 0;

    /**
   * Update the properties of this <code>QExtOsgiConfiguration</code> object.
   *
   ...
   * <code>QExtOsgiConfigurationListener</code>s with a
   * <code>QExtOsgiConfigurationEvent::CM_UPDATED</code> event.
   *
   * @param properties the new set of properties for this configuration
   * @throws QExtIOException if update cannot be made persistent
   * @throws QExtInvalidArgumentException if the <code>QExtOsgiDictionary</code> object
   *         contains invalid configuration types or contains case variants of
   *         the same key name.
   * @throws QExtIllegalStateException if this configuration has been deleted
   */
    virtual void update(const QExtOsgiDictionary& properties) = 0;

    /**
   * Delete this <code>QExtOsgiConfiguration</code> object.
   *
   * Removes this configuration object from the persistent store. Notify
   * asynchronously the corresponding Managed Service or Managed Service
   * Factory. A <code>QExtOsgiManagedService</code> object is notified by a call to
   * its <code>updated</code> method with a <code>null</code> properties
   * argument. A <code>QExtOsgiManagedServiceFactory</code> object is notified by a
   * call to its <code>deleted</code> method.
   *
   * <p>
   * Also initiates an asynchronous call to all
   * <code>QExtOsgiConfigurationListener</code>s with a
   * <code>QExtOsgiConfigurationEvent::CM_DELETED</code> event.
   *
   * @throws QExtIOException If delete fails
   * @throws QExtIllegalStateException if this configuration has been deleted
   */
   ...
   */
    ...
    /**
   * Update the <code>QExtOsgiConfiguration</code> object with the current
   * properties.
   *
   ...
   * Factory via its <code>QExtOsgiConfigurationPlugin</code> object.
   *
   * @see QExtOsgiConfigurationPlugin
   * @throws QExtIOException if update cannot access the properties in persistent
   *         storage
   * @throws QExtIllegalStateException if this configuration has been deleted
   */
    ...
    /**
   * Bind this <code>QExtOsgiConfiguration</code> object to the specified plugin
   * location.
   *
   * If the pluginLocation parameter is <code>null</code> then the
   * <code>QExtOsgiConfiguration</code> object will not be bound to a location. It
   * will be set to the plugin's location before the first time a Managed
   * Service/Managed Service Factory receives this <code>QExtOsgiConfiguration</code>
   ...
   * @throws QExtIllegalStateException If this configuration has been deleted.
   * @throws QExtSecurityException If the caller does not have
   ...
   *         <code>null</code>.
   * @throws QExtIllegalStateException If this <code>Configuration</code> object
   *         has been deleted.
   * @throws QExtSecurityException If the caller does not have
   *         <code>QExtOsgiConfigurationPermission[*,CONFIGURE]</code>.
   */
   ...
   *
   * @param other <code>QExtOsgiConfiguration</code> object to compare against
   * @return <code>true</code> if equal, <code>false</code> if the PID's differ.
   */
    bool operator==(const QExtOsgiConfiguration& other) const;
...
typedef QSharedPointer<QExtOsgiConfiguration> QExtOsgiConfigurationPtr;

/**
 * Hash code is based on PID.
 *
 * The hashcode for two QExtOsgiConfiguration objects must be the same when the
 * Configuration PID's are the same.
 *
 * @param configuration The configuration object for which to compute the hash value.
 * @return hash code for this QExtOsgiConfiguration object
 */
QEXT_OSGI_API uint qHash(QExtOsgiConfigurationPtr configuration);

QEXT_OSGI_API bool operator==(const QExtOsgiConfigurationPtr& c1, const QExtOsgiConfigurationPtr c2);

/** @}*/

#endif // _QEXTOSGICONFIGURATION_H
