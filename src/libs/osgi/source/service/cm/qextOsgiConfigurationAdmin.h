...
#ifndef _QEXTOSGICONFIGURATIONADMIN_H
#define _QEXTOSGICONFIGURATIONADMIN_H

#include <qext/osgi/service/cm/qextOsgiConfiguration.h>
 ...
 * persistently. This information is represented in <code>QExtOsgiConfiguration</code>
 * objects. The actual configuration data is a <code>QExtOsgiDictionary</code> of
 * properties inside a <code>QExtOsgiConfiguration</code> object.
 *
 * <p>
 ...
 * will maintain 0 or more <code>QExtOsgiConfiguration</code> objects for a Managed
 ...
 * QExtOsgiManagedService#updated method with the new properties. The
 ...
 * the Managed Service Factory. For each such <code>QExtOsgiConfiguration</code>
 * objects, it calls the <code>QExtOsgiManagedServiceFactory#updated</code> method
 * asynchronously with the new properties. The calls to the <code>%updated</code>
 * method of a <code>QExtOsgiManagedServiceFactory</code> must be executed sequentially
 ...
 * <code>QExtOsgiConfigurationPermission[*,CONFIGURE]</code>.
 *
 * <p>
 * <code>QExtOsgiConfiguration</code> objects can be <i>bound </i> to a specified plugin
 ...
 * log an error. In the case where a <code>QExtOsgiConfiguration</code> object is not
 ...
 * property. When a <code>QExtOsgiConfiguration</code> object is bound to a plugin
 ...
 * <code>QExtOsgiConfiguration</code> object is unbound, that is its location field is
 ...
 * <code>QExtOsgiConfigurationAdmin</code> must use a
 * {@link QExtOsgiServiceFactory} to support this concept.
 *
 */
struct QEXT_OSGI_API QExtOsgiConfigurationAdmin
{
   ...
   * associated with a <code>QExtOsgiConfiguration</code> object. This property can
   ...
   */
    ...
    /**
   * Create a new factory <code>QExtOsgiConfiguration</code> object with a new PID.
   *
   * The properties of the new <code>QExtOsgiConfiguration</code> object are
   * <code>null</code> until the first time that its
   * {@link QExtOsgiConfiguration#update(const QExtOsgiDictionary&)} method is called.
   *
   * <p>
   * It is not required that the <code>factoryPid</code> maps to a
   * registered Managed Service Factory.
   * <p>
   * The <code>QExtOsgiConfiguration</code> object is bound to the location of the
   * calling plugin.
   *
   * @param factoryPid PID of factory (not <code>null</code>).
   * @return A new <code>QExtOsgiConfiguration</code> object.
   * @throws QExtIOException if access to persistent storage fails.
   * @throws QExtSecurityException if caller does not have
   *         <code>QExtOsgiConfigurationPermission[*,CONFIGURE]</code> and
   *         <code>factoryPid</code> is bound to another plugin.
   */
    virtual QExtOsgiConfigurationPtr createFactoryConfiguration(const QString& factoryPid) = 0;
   ...
   *
   * <p>
   * The <code>QExtOsgiConfiguration</code> is bound to the location specified. If
   ...
   * @return a new <code>QExtOsgiConfiguration</code> object.
   * @throws QExtIOException if access to persistent storage fails.
   * @throws QExtSecurityException if caller does not have <code>QExtOsgiConfigurationPermission[*,CONFIGURE]</code>.
   */
    virtual QExtOsgiConfigurationPtr createFactoryConfiguration(const QString& factoryPid, const QString& location) = 0;

    /**
   * Get an existing <code>QExtOsgiConfiguration</code> object from the persistent
   * store, or create a new <code>QExtOsgiConfiguration</code> object.
   *
   * <p>
   * If a <code>QExtOsgiConfiguration</code> with this PID already exists in
   * Configuration Admin service return it. The location parameter is ignored
   * in this case.
   *
   * <p>
   * Else, return a new <code>QExtOsgiConfiguration</code> object. This new object
   ...
   * @return An existing or new <code>QExtOsgiConfiguration</code> object.
   * @throws QExtIOException if access to persistent storage fails.
   * @throws QExtSecurityException if the caller does not have <code>QExtOsgiConfigurationPermission[*,CONFIGURE]</code>.
   */
    virtual QExtOsgiConfigurationPtr getConfiguration(const QString& pid, const QString& location) = 0;

    /**
   * Get an existing or new <code>QExtOsgiConfiguration</code> object from the
   * persistent store.
   *
   * If the <code>QExtOsgiConfiguration</code> object for this PID does not exist,
   * create a new <code>QExtOsgiConfiguration</code> object for that PID, where
   * properties are <code>null</code>. Bind its location to the calling
   * plugin's location.
   *
   * <p>
   * Otherwise, if the location of the existing <code>QExtOsgiConfiguration</code> object
   * is <code>null</code>, set it to the calling plugin's location.
   *
   * @param pid persistent identifier.
   * @return an existing or new <code>QExtOsgiConfiguration</code> matching the PID.
   * @throws QExtIOException if access to persistent storage fails.
   * @throws QExtSecurityException if the <code>QExtOsgiConfiguration</code> object is
   *         bound to a location different from that of the calling plugin and it
   *         has no <code>QExtOsgiConfigurationPermission[*,CONFIGURE]</code>.
   */
    virtual QExtOsgiConfigurationPtr getConfiguration(const QString& pid) = 0;

    /**
   * List the current <code>QExtOsgiConfiguration</code> objects which match the
   * filter.
   *
   * <p>
   * Only <code>QExtOsgiConfiguration</code> objects with non- <code>null</code>
   * properties are considered current. That is,
   * <code>QExtOsgiConfiguration#getProperties()</code> is guaranteed not to return
   * an empty list for each of the returned <code>QExtOsgiConfiguration</code>
   * objects.
   *
   * <p>
   * Normally only <code>QExtOsgiConfiguration</code> objects that are bound to the
   * location of the calling plugin are returned, or all if the caller has
   * <code>QExtOsgiConfigurationPermission[*,CONFIGURE]</code>.
   *
   * <p>
   * The syntax of the filter string is as defined in the
   * {@link QExtOsgiLDAPSearchFilter} class. The filter can test any
   ...
   * <code>QExtOsgiConfiguration</code> objects should be returned.
   *
   * @param filter A filter string, or an empty string to retrieve all
   *        <code>QExtOsgiConfiguration</code> objects.
   * @return All matching <code>QExtOsgiConfiguration</code> objects, or
   *         an empty list if there aren't any.
   * @throws QExtIOException if access to persistent storage fails
   * @throws QExtInvalidArgumentException if the filter string is invalid
   */
    virtual QList<QExtOsgiConfigurationPtr> listConfigurations(const QString& filter = QString()) = 0;
};

Q_DECLARE_INTERFACE(QExtOsgiConfigurationAdmin, "org.qext.service.cm.ConfigurationAdmin")

#endif // _QEXTOSGICONFIGURATIONADMIN_H
