...
#ifndef _QEXTOSGIEVENTADMIN_H
#define _QEXTOSGIEVENTADMIN_H
 ...
 *
 */
struct QExtOsgiEventAdmin
{
    virtual ~QExtOsgiEventAdmin() {}
   ...
   *
   */
    virtual void postEvent(const QExtOsgiEvent& event) = 0;
   ...
   *
   */
    virtual void sendEvent(const QExtOsgiEvent& event) = 0;
   ...
   * following signature:
   * \code
   * someSignal(const QExtOsgiDictionary& props)
   * \endcode
   * where <code>props</code> will be used to construct a QExtOsgiEvent class which
   ...
   *
   * Slots should be registered with a property QExtOsgiEventConstants::EVENT_TOPIC.
   ...
   * \endverbatim
   *
   * Slots can also be registered with a property named QExtOsgiEventConstants::EVENT_FILTER.
   ...
   *
   * @throws QExtInvalidArgumentException If <code>subscriber</code> or <code>member</code> is 0
   ...
   */
    virtual qlonglong subscribeSlot(const QObject* subscriber, const char* member,
                                    const QExtOsgiDictionary& properties, Qt::ConnectionType type = Qt::AutoConnection) = 0;
   ...
   */
    virtual bool updateProperties(qlonglong subscriptionId, const QExtOsgiDictionary& properties) = 0;

};


Q_DECLARE_INTERFACE(QExtOsgiEventAdmin, "org.qext.service.event.EventAdmin")

#endif // _QEXTOSGIEVENTADMIN_H
