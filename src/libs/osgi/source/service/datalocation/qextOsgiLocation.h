...
#ifndef _QEXTOSGILOCATION_H
#define _QEXTOSGILOCATION_H
...
/**
 * A QExtOsgiLocation represents a QUrl which may have a default value, may be read only, may
 * or may not have a current value and may be cascaded on to a parent location.
 */
struct QEXT_OSGI_API QExtOsgiLocation
{
   ...
   */
    static const QString QEXT_OSGI_HOME_FILTER;


    virtual ~QExtOsgiLocation() {}
   ...
   */
    virtual QExtOsgiLocation* getParentLocation() const = 0;
   ...
   * @throws QExtIllegalStateException if the location's value is already set
   * @throws QExtRuntimeException if there was an unexpected problem while acquiring the lock
   */
   ...
   *
   * @exception QExtRuntimeException if there was an unexpected problem while acquiring the lock
   */
    virtual bool lock() = 0;
   ...
   * @throws QExtRuntimeException if there was an unexpected problem reading the lock
   */
   ...
   */
    virtual QExtOsgiLocation* createLocation(QExtOsgiLocation* parent, const QUrl& defaultValue, bool readonly) = 0;
   ...
   * to the returned data area.  An <code>QExtRuntimeException</code> will be thrown if
   ...
   * @throws QExtRuntimeException if the location URL is not already set
   */
    virtual QUrl getDataArea(const QString& path) const = 0;

};

Q_DECLARE_INTERFACE(QExtOsgiLocation, "org.qext.service.datalocation.Location")

#endif // _QEXTOSGILOCATION_H
