...
static const QString CTK = "qext";
static const QString PRODUCT_SITE_MARKER = ".qextproduct";
...
static QScopedPointer<QExtOsgiBasicLocation> installLocation;
static QScopedPointer<QExtOsgiBasicLocation> configurationLocation;
static QScopedPointer<QExtOsgiBasicLocation> userLocation;
                                     ...
                                     bool computeReadOnly, const QString& dataAreaPrefix)
{
    ...
    QVariant userReadOnlySetting = QExtOsgiPluginFrameworkProperties::getProperty(property + QExtOsgiLocationManager::READ_ONLY_AREA_SUFFIX);
    bool readOnly = (userReadOnlySetting.isNull() ? readOnlyDefault : userReadOnlySetting.toBool());
    // if the instance location is not set, predict where the workspace will be and
    // put the instance area inside the workspace meta area.
    if (location.isNull())
    {
        return new QExtOsgiBasicLocation(property, defaultLocation,
    ...
    {
        return new QExtOsgiBasicLocation(property, QUrl(), readOnly, dataAreaPrefix);
    }
    if (trimmedLocation.startsWith(USER_HOME))
    {
        QString base = SubstituteVar(location, USER_HOME, QExtOsgiPluginFrameworkLauncher::PROP_USER_HOME);
        location = QFileInfo(QDir(base), userDefaultAppendage).absoluteFilePath();
    }
    else if (trimmedLocation.startsWith(USER_DIR))
    {
        QString base = SubstituteVar(location, USER_DIR, QExtOsgiPluginFrameworkLauncher::PROP_USER_DIR);
        location = QFileInfo(QDir(base), userDefaultAppendage).absoluteFilePath();
    }
    int idx = location.indexOf(INSTALL_HASH_PLACEHOLDER);
    if (idx == 0)
    {
        throw QExtRuntimeException("The location cannot start with '" + INSTALL_HASH_PLACEHOLDER + "': " + location);
    }
    else if (idx > 0)
    {
    ...
    QExtOsgiBasicLocation* result = NULL;
    if (url.isValid())
    {
        result = new QExtOsgiBasicLocation(property, QUrl(), !userReadOnlySetting.isNull() || !computeReadOnly ? readOnly : !CanWrite(url), dataAreaPrefix);
...
{
    QString property = QExtOsgiPluginFrameworkProperties::getProperty(QExtOsgiPluginFrameworkLauncher::PROP_INSTALL_AREA).toString();
    if (!property.isEmpty())
    {
...
{
    QVariant property = QExtOsgiPluginFrameworkProperties::getProperty(QExtOsgiPluginFrameworkLauncher::PROP_SHARED_CONFIG_AREA);
    if (property.isNull())
    {
        return QUrl();
    }
    ...
    QExtOsgiPluginFrameworkProperties::setProperty(QExtOsgiPluginFrameworkLauncher::PROP_SHARED_CONFIG_AREA, sharedConfigurationURL.toString());
    return QUrl();
}
...
{
    // 1) We store the config state relative to the 'qext' directory if possible
...
static QString ComputeDefaultUserAreaLocation(const QString& pathAppendage)
{
    //    we store the state in <user.home>/.qext/<application-id>_<version> where <user.home>
    //    is unique for each local user, and <application-id> is the one
    //    defined in .qextproduct marker file. If .qextproduct does not
    //    exist, use "qext" as the application-id.
    QString installProperty = QExtOsgiPluginFrameworkProperties::getProperty(QExtOsgiPluginFrameworkLauncher::PROP_INSTALL_AREA).toString();
    ...
    QString appName = "." + CTK;
    QFileInfo qextProduct(QDir(installDir.absoluteFilePath()), PRODUCT_SITE_MARKER);
    if (qextProduct.exists())
    {
        QSettings props(qextProduct.absoluteFilePath(), QSettings::IniFormat);
    ...
    QString userHome = QExtOsgiPluginFrameworkProperties::getProperty(QExtOsgiPluginFrameworkLauncher::PROP_USER_HOME).toString();
...
const QString QExtOsgiLocationManager::READ_ONLY_AREA_SUFFIX = ".readOnly";

// configuration area file/dir names
const QString QExtOsgiLocationManager::CONFIG_FILE = "config.ini";
const QString QExtOsgiLocationManager::QEXT_OSGI_PROPERTIES = "qextOsgi.properties";


//----------------------------------------------------------------------------
void QExtOsgiLocationManager::initializeLocations()
{
    if (isInitialized) return;

    // set the qext storage area if it exists
    QVariant osgiStorage = QExtOsgiPluginFrameworkProperties::getProperty(QExtOsgiPluginConstants::FRAMEWORK_STORAGE);
    if (!osgiStorage.isNull())
    {
        QExtOsgiPluginFrameworkProperties::setProperty(QExtOsgiPluginFrameworkLauncher::PROP_CONFIG_AREA, osgiStorage);
    }
    // do install location initialization first since others may depend on it
    // assumes that the property is already set
    installLocation.reset(BuildLocation(QExtOsgiPluginFrameworkLauncher::PROP_INSTALL_AREA, QUrl(), "", true, false, QString()));

    // TODO not sure what the data area prefix should be here for the user area
    QScopedPointer<QExtOsgiBasicLocation> temp(BuildLocation(QExtOsgiPluginFrameworkLauncher::PROP_USER_AREA_DEFAULT,
                                                             QUrl(), "", false, false, QString()));
    QUrl defaultLocation = temp ? temp->getUrl() : QUrl();
    if (!defaultLocation.isValid())
    {
        defaultLocation = BuildUrl(QFileInfo(QDir(QExtOsgiPluginFrameworkProperties::getProperty(QExtOsgiPluginFrameworkLauncher::PROP_USER_HOME).toString()), "user").absoluteFilePath(), true);
    }
    userLocation.reset(BuildLocation(QExtOsgiPluginFrameworkLauncher::PROP_USER_AREA, defaultLocation, "", false, false, QString()));

    temp.reset(BuildLocation(QExtOsgiPluginFrameworkLauncher::PROP_INSTANCE_AREA_DEFAULT, QUrl(), "", false, false, INSTANCE_DATA_AREA_PREFIX));
    defaultLocation = temp ? temp->getUrl() : QUrl();
    if (!defaultLocation.isValid())
    {
        defaultLocation = BuildUrl(QFileInfo(QDir(QExtOsgiPluginFrameworkProperties::getProperty(QExtOsgiPluginFrameworkLauncher::PROP_USER_DIR).toString()), "workspace").absoluteFilePath(), true);
    }
    instanceLocation.reset(BuildLocation(QExtOsgiPluginFrameworkLauncher::PROP_INSTANCE_AREA, defaultLocation, "", false, false, INSTANCE_DATA_AREA_PREFIX));

    //mungeConfigurationLocation();

    // compute a default but it is very unlikely to be used since main will have computed everything
    temp.reset(BuildLocation(QExtOsgiPluginFrameworkLauncher::PROP_CONFIG_AREA_DEFAULT, QUrl(), "", false, false, QString()));
    defaultLocation = temp ? temp->getUrl() : QUrl();
    if (!defaultLocation.isValid() && QExtOsgiPluginFrameworkProperties::getProperty(QExtOsgiPluginFrameworkLauncher::PROP_CONFIG_AREA).isNull())
    {
    ...
    configurationLocation.reset(BuildLocation(QExtOsgiPluginFrameworkLauncher::PROP_CONFIG_AREA, defaultLocation, "", false, false, QString()));
    ...
    {
        QExtOsgiBasicLocation* parent = new QExtOsgiBasicLocation(QString(), parentLocation, true, QString());
        configurationLocation->setParent(parent);
    }
    //initializeDerivedConfigurationLocations();

    if (QExtOsgiPluginFrameworkProperties::getProperty(QExtOsgiPluginFrameworkLauncher::PROP_HOME_LOCATION_AREA).isNull())
    {
        QString blueberryHomeLocationPath = ComputeCTKHomeLocation();
        if (!blueberryHomeLocationPath.isEmpty())
        {
            QExtOsgiPluginFrameworkProperties::setProperty(QExtOsgiPluginFrameworkLauncher::PROP_HOME_LOCATION_AREA, blueberryHomeLocationPath);
        }
    }
    // if qext.home.location is not set then default to qext.install.area
    if (QExtOsgiPluginFrameworkProperties::getProperty(QExtOsgiPluginFrameworkLauncher::PROP_HOME_LOCATION_AREA).isNull() &&
        !QExtOsgiPluginFrameworkProperties::getProperty(QExtOsgiPluginFrameworkLauncher::PROP_INSTALL_AREA).isNull())
    {
        QExtOsgiPluginFrameworkProperties::setProperty(QExtOsgiPluginFrameworkLauncher::PROP_HOME_LOCATION_AREA,
                                                       QExtOsgiPluginFrameworkProperties::getProperty(QExtOsgiPluginFrameworkLauncher::PROP_INSTALL_AREA));
    }
    qextHomeLocation.reset(BuildLocation(QExtOsgiPluginFrameworkLauncher::PROP_HOME_LOCATION_AREA, QUrl(), "", true, true, QString()));

    isInitialized = true;
}

//----------------------------------------------------------------------------
QExtOsgiBasicLocation* QExtOsgiLocationManager::getUserLocation()
{
    return userLocation.data();
}

//----------------------------------------------------------------------------
QExtOsgiBasicLocation* QExtOsgiLocationManager::getConfigurationLocation()
{
    return configurationLocation.data();
}

//----------------------------------------------------------------------------
QExtOsgiBasicLocation* QExtOsgiLocationManager::getInstallLocation()
{
    return installLocation.data();
}

//----------------------------------------------------------------------------
QExtOsgiBasicLocation* QExtOsgiLocationManager::getInstanceLocation()
{
    return instanceLocation.data();
}

//----------------------------------------------------------------------------
QExtOsgiBasicLocation* QExtOsgiLocationManager::getCTKHomeLocation()
{
    return qextHomeLocation.data();
}
