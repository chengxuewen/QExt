...
#ifndef _QEXTOSGIPLUGINSTORAGESQL_P_H
#define _QEXTOSGIPLUGINSTORAGESQL_P_H
...
class QExtOsgiPluginArchiveSQL;

/**
 * \ingroup PluginFramework
 */
class QExtOsgiPluginStorageSQL : public QExtOsgiPluginStorage
{
...
public:
   ...
   *
   */
    QExtOsgiPluginStorageSQL(QExtOsgiPluginFrameworkContext* framework);

    virtual ~QExtOsgiPluginStorageSQL();
   ...
   * @param createArchive If \c true (default) a new QExtOsgiPluginArchive instance is returned.
   *
   ...
   */
    void replacePluginArchive(QSharedPointer<QExtOsgiPluginArchive> oldPA, QSharedPointer<QExtOsgiPluginArchive> newPA);

    /**
   * Removes all persisted data related to the given QExtOsgiPluginArchive.
   *
   * @throws QExtOsgiPluginDatabaseException
   */
    bool removeArchive(QSharedPointer<QExtOsgiPluginArchive> pa);
   ...
   */
    QList<QSharedPointer<QExtOsgiPluginArchive> > getAllPluginArchives() const;
   ...
   */
    ...
    /**
   * Closes the plugin database. Throws a QExtOsgiPluginDatabaseException
   ...
   */
    ...
    // -------------------------------------------------------------
    // end QExtOsgiPluginStorage interface
    // -------------------------------------------------------------
   ...
   */
    ...
    /**
   * Removes all persisted data related to the given QExtOsgiPluginArchiveSQL.
   * This is identical to removeArchive(QSharedPointer<QExtOsgiPluginArchive>).
   *
   * @throws QExtOsgiPluginDatabaseException
   */
    bool removeArchive(QExtOsgiPluginArchiveSQL* pa);
...
private:
   ...
   * @see getDatabasePath()
   * @see QExtOsgiPluginDatabaseException
   *
   * @throws QExtOsgiPluginDatabaseException
   */
    void open();
   ...
   */
    ...
    /**
   * Find position for QExtOsgiPluginArchive with specified id
   *
   ...
   */
    ...
    /**
   * Find position for QExtOsgiPluginArchive
   *
   * @param id Plugin archive id to find.
   * @return Position in the m_archives List.
   */
    int find(QExtOsgiPluginArchive* pa) const;

    void initNextFreeIds();

    /**
   * Reads the persisted plugin data and creates a QExtOsgiPluginArchive object
   ...
   */
    ...
    void insertArchive(QSharedPointer<QExtOsgiPluginArchiveSQL> pa);

    void insertArchive(QSharedPointer<QExtOsgiPluginArchiveSQL> pa, QSqlQuery* query);
   ...
   */
    QList<QSharedPointer<QExtOsgiPluginArchive> > m_archives;

    /**
   * Framework handle.
   */
    QExtOsgiPluginFrameworkContext* m_framework;
...
#endif // _QEXTOSGIPLUGINSTORAGESQL_P_H
