...
#ifndef _QEXTOSGIAPPLICATIONDESCRIPTOR_H
#define _QEXTOSGIAPPLICATIONDESCRIPTOR_H
...
struct QExtOsgiApplicationHandle;
 ...
 * creation.
 */
struct QEXT_OSGI_API QExtOsgiApplicationDescriptor
{
   ...
   */
    static const QString APPLICATION_PID; // = QExtOsgiPluginConstants::SERVICE_PID;
   ...
   */
    static const QString APPLICATION_VENDOR; // = QExtOsgiPluginConstants::SERVICE_VENDOR;
   ...
   */
    ...
    virtual ~QExtOsgiApplicationDescriptor() {}
   ...
   * properties of this <code>QExtOsgiApplicationDescriptor</code> as well.
   * <p>
   ...
   *
   * @throws QExtIllegalStateException
   *             if the application descriptor is unregistered
   */
    virtual QExtOsgiProperties getProperties(const QLocale& locale) const = 0;
   ...
   *             if the application descriptor is unregistered
   */
    virtual QExtOsgiProperties getProperties() const = 0;
   ...
   * {@link QExtOsgiApplicationException} with the reason code
   * {@link QExtOsgiApplicationException#APPLICATION_LOCKED}.
   * <li>Create and start an application instance.
   * <li>Returns the <code>QExtOsgiApplicationHandle</code>.
   * </ul>
   * <p>
   ...
   * CTK defines the <code>org.qext.triggeringevent</code> key to be used to pass
   ...
   *        null
   *
   * @return the registered QExtOsgiApplicationHandle, which represents the newly
   *         launched application instance. Never returns <code>null</code>.
   *
   * @throws QExtOsgiApplicationException if starting the application failed
   * @throws QExtIllegalStateException if the application descriptor is
   *         unregistered
   * @throws std::invalid_argument if the specified <code>QHash</code> contains
   *         invalid keys (empty <code>QString</code>)
   */
    virtual QExtOsgiApplicationHandle* launch(const QHash<QString, QVariant> &arguments) = 0;

};

Q_DECLARE_INTERFACE(QExtOsgiApplicationDescriptor, "org.qext.service.application.ApplicationDescriptor")

#endif // _QEXTOSGIAPPLICATIONDESCRIPTOR_H
