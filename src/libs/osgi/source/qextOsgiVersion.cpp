...
const QString QExtOsgiVersion::SEPARATOR = ".";
const QRegExp QExtOsgiVersion::RegExp = QRegExp("[a-zA-Z0-9_\\-]*");

//----------------------------------------------------------------------------
QExtOsgiVersion QExtOsgiVersion::emptyVersion()
{
    static QExtOsgiVersion emptyV(false);
    return emptyV;
}

//----------------------------------------------------------------------------
QExtOsgiVersion QExtOsgiVersion::undefinedVersion()
{
    static QExtOsgiVersion undefinedV(true);
    return undefinedV;
}

//----------------------------------------------------------------------------
QExtOsgiVersion& QExtOsgiVersion::operator=(const QExtOsgiVersion& v)
{
...
//----------------------------------------------------------------------------
QExtOsgiVersion::QExtOsgiVersion(bool undefined)
...
//----------------------------------------------------------------------------
void QExtOsgiVersion::validate()
{
    if (!RegExp.exactMatch(qualifier))
        throw QExtInvalidArgumentException(QString("invalid qualifier: ") + qualifier);

    undefined = false;
}

//----------------------------------------------------------------------------
QExtOsgiVersion::QExtOsgiVersion(unsigned int majorVersion, unsigned int minorVersion, unsigned int microVersion)
    ...
    : majorVersion(0), minorVersion(0), microVersion(0), undefined(true)
{
    ...
    if (!ok) throw QExtInvalidArgumentException("invalid format");
...
//----------------------------------------------------------------------------
QExtOsgiVersion::QExtOsgiVersion(const QExtOsgiVersion& version)
...
QExtOsgiVersion QExtOsgiVersion::parseVersion(const QString &version)
{
    ...
    return QExtOsgiVersion(version2);
}

//----------------------------------------------------------------------------
bool QExtOsgiVersion::isUndefined() const
{
    return undefined;
}

//----------------------------------------------------------------------------
unsigned int QExtOsgiVersion::getMajor() const
{
    if (undefined) throw QExtIllegalStateException("Version undefined");
    return majorVersion;
}

//----------------------------------------------------------------------------
unsigned int QExtOsgiVersion::getMinor() const
{
    if (undefined) throw QExtIllegalStateException("Version undefined");
    return minorVersion;
}
...
//----------------------------------------------------------------------------
unsigned int QExtOsgiVersion::getMicro() const
{
    if (undefined) throw QExtIllegalStateException("Version undefined");
    return microVersion;
}
...
//----------------------------------------------------------------------------
QString QExtOsgiVersion::getQualifier() const
{
    if (undefined) throw QExtIllegalStateException("Version undefined");
    return qualifier;
}
...
//----------------------------------------------------------------------------
QString QExtOsgiVersion::toString() const
{
...
//----------------------------------------------------------------------------
bool QExtOsgiVersion::operator==(const QExtOsgiVersion& other) const
{
    if (&other == this)
    { // quicktest
        return true;
    ...
    if (this->undefined) throw QExtIllegalStateException("Version undefined");
...
//----------------------------------------------------------------------------
int QExtOsgiVersion::compare(const QExtOsgiVersion& other) const
{
    if (&other == this)
    { // quicktest
        return 0;
    ...
    if (this->undefined || other.undefined)
        throw QExtIllegalStateException("Cannot compare undefined version");
...
//----------------------------------------------------------------------------
bool QExtOsgiVersion::operator <(const QExtOsgiVersion &object) const
{
    return this->compare(object) < 0;
}

//----------------------------------------------------------------------------
QDebug operator<<(QDebug dbg, const QExtOsgiVersion& v)
{
    dbg << v.toString();
