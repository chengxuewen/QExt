...
{
    QEXTTcpAbstractPacket *packet = m_tcpClient->dequeuePacket();
    if (packet) {
        QEXTTcpAbstractPacket::printPacket(packet);
    }
}
...
void CMainWindow::on_pushButton_send_clicked()
{
    ...
    QEXTTcpAbstractPacket *packet = m_tcpClient->packetParser()->createPacket(m_tcpClient->socket()->identityId());
