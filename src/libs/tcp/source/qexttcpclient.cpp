...
void QEXTTcpClientPrivate::initClient(QEXTTcpClient *client)
{
    ...
    QObject::connect(m_socket.data(), SIGNAL(stateChanged(QAbstractSocket::SocketState)),
                     client, SIGNAL(socketStateChanged(QAbstractSocket::SocketState)), Qt::QueuedConnection);
...
{
    if (connect) {
        QObject::connect(proxy, SIGNAL(newSendPacketReady(QEXTTcpAbstractPacket*)),
                         m_socket.data(), SLOT(enqueueSendPacket(QEXTTcpAbstractPacket*)));
    } else {
        QObject::disconnect(proxy, SIGNAL(newSendPacketReady(QEXTTcpAbstractPacket*)),
                            m_socket.data(), SLOT(enqueueSendPacket(QEXTTcpAbstractPacket*)));
    }
}
...
QEXTTcpClient::QEXTTcpClient(QEXTTcpAbstractPacketParser *packetParser,
                             QEXTTcpAbstractPacketDispatcher *packetDispatcher,
                             QObject *parent)
    : QEXTTcpClientProxy(*(new QEXTTcpClientPrivate(this)), parent)
{
    QEXT_D(QEXTTcpClient);
...
QEXTTcpClient::QEXTTcpClient(QEXTTcpClientPrivate &dd,
                             QEXTTcpAbstractPacketParser *packetParser,
                             QObject *parent)
    : QEXTTcpClientProxy(dd, parent)
{
    QEXT_D(QEXTTcpClient);
