...
bool QEXTTcpTask::run()
{
    ...
    if (!d->m_taskPool.isNull()) {
        ...
        QEXTTcpAbstractPacketParser *packetParser = d->m_taskPool->packetParser();
        QEXTTcpAbstractPacket *notifyPacket = packetParser->createPacket(d->m_packet->socketId());
        ...
        if (packetParser->isRequestPacket(d->m_packet.data())) {
            ...
            QEXTTcpAbstractPacket *replyPacket = packetParser->createReplyPacket(d->m_packet.data());
