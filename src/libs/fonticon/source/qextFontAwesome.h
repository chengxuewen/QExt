#ifndef _QEXTICONFONTAWESOME_H
#define _QEXTICONFONTAWESOME_H

#include <qextFontIconGlobal.h>
...
class QExtFontAwesomeIconPainterInterface;

/**
 * \brief The QExtFontAwesome class
 * NOTE: Though the name is QExtFontAwesome and currently it's very Font Awesome based,
 ...
 *
 * Usage:
 *      You probably want to create a single QExtFontAwesome object for your whole application:
 *      <code/>
 *          QExtFontAwesome* awesome = new QExtFontAwesome( qApp )
 *          awesome->initFontAwesome();     // This line is important as it loads the font and initializes the named icon map
 *      </code>
 *
 * Example:
 *      <code/>
 *          // You should create a single object of QExtFontAwesome.
 *          QExtFontAwesome* awesome = new QExtFontAwesome( qApp );
 ...
 *
 *          // You can also use 'string' names to access the icons. (The string version omits
 *          // the 'fa-' or 'icon-' prefix and has no dashes )
 ...
 *          {
 *          public:
 *              virtual void paint( QExtFontAwesome* awesome, QPainter* painter, const QRect& rectIn, QIcon::Mode mode, QIcon::State state, const QVariantMap& options  )
 *              {
 ...
 *
 * Default options:
 *      The following options are default in the QExtFontAwesome class.
 *      <code/>
 ...
 * the QExtFontAwesome object. After that the options are expanded/overwritten by the options supplied to the icon.
 *
 ...
 * text-selected-off
 */
class QEXT_FONTICON_API QExtFontAwesome : public QObject
{
    Q_OBJECT
public:
    enum OptionType
    {
        Option_ScaleFactor = 0,
        Option_Color,
        Option_ColorOff,
        Option_ColorDisabled,
        Option_ColorDisabledOff,
        Option_ColorActive,
        Option_ColorActiveOff,
        Option_ColorSelected,
        Option_ColorSelectedOff,
        Option_Text,
        Option_TextOff,
        Option_TextDisabled,
        Option_TextDisabledOff,
        Option_TextActive,
        Option_TextActiveOff,
        Option_TextSelected,
    ...
    explicit QExtFontAwesome(QObject *parent = QEXT_NULLPTR);
    virtual ~QExtFontAwesome();

    static int maiorVersion();
    static int minorVersion();
    Q_INVOKABLE QString version() const;
    Q_INVOKABLE QString fontName() const;

    Q_INVOKABLE QString textFromKey(const QString &key) const;
    Q_INVOKABLE QString textFromValue(const int &type) const;
    Q_INVOKABLE QList<QString> fontNamelist();
    Q_INVOKABLE QList<QString> fontTextlist();
    Q_INVOKABLE FontNameTextMap fontNameTextMap();

    /**
     * \brief QExtFontAwesome::setDefaultOption Sets a default option.
    ...
    /**
     * \brief QExtFontAwesome::defaultOption
     * \param name
    ...
    /**
     * \brief QExtFontAwesome::icon Creates an icon with the given code-point
     *      <code>
     ...
     * \return
     */
    QIcon icon(QExtFontAwesomeIconPainterInterface *painter, const FontOptionValueMap &optionMap = FontOptionValueMap());

    /**
     * \brief QExtFontAwesome::give Adds a named icon-painter to the QExtFontAwesome icon map
     * As the name applies the ownership is passed over to QExtFontAwesome
     * \param name the name of the icon
     * \param paintert he icon painter to add for this name
     */
    void give(const QString &name, QExtFontAwesomeIconPainterInterface *painter);

    /**
     * \brief QExtFontAwesome::font Creates/Gets the icon font with a given size in pixels.
...
private:
    QString m_fontName;                           ///< The font name used for this map
    ...
    QExtFontAwesomeIconPainterInterface *m_fontIconPainter;               ///< A special painter fo painting codepoints
    QHash<QString, QExtFontAwesomeIconPainterInterface *> m_painterMap; ///< A map of custom painters
};

/// The QExtFontAwesomeIconPainter is a specialized painter for painting icons
/// your can implement an iconpainter to create custom font-icon code
class QExtFontAwesomeIconPainterInterface
{
public:
    virtual ~QExtFontAwesomeIconPainterInterface() {}

    virtual void paint(QExtFontAwesome *awesome, QPainter *painter, const QRect &rect, QIcon::Mode mode,
                       QIcon::State state, const QExtFontAwesome::FontOptionValueMap &options) = 0;
};

#define qextFontAwesome qextGlobalFontAwesome()
QEXT_FONTICON_API QExtFontAwesome *qextGlobalFontAwesome();
