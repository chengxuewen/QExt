...
struct ViewModelController::ViewModelControllerImpl {
    ...
    std::map<QEXTMvvmSessionItem*, ViewItem*> item_to_view; //! correspondence of item and its view
    Path root_item_path;
    ...
    void iterate(const QEXTMvvmSessionItem* item, ViewItem* parent)
    {
    ...
    void insert_view(QEXTMvvmSessionItem* parent, const TagRow& tagrow)
    {
    ...
    std::vector<ViewItem*> findViews(const QEXTMvvmSessionItem* item) const
    {
    ...
    , p_impl(std::make_unique<ViewModelControllerImpl>(this, view_model))
{
    ...
    auto on_item_inserted = [this](QEXTMvvmSessionItem* item, TagRow tagrow) {
        onItemInserted(item, std::move(tagrow));
    };
    setOnItemInserted(on_item_inserted);

    auto on_item_removed = [this](QEXTMvvmSessionItem* item, TagRow tagrow) {
        onItemRemoved(item, std::move(tagrow));
    };
    setOnItemRemoved(on_item_removed);

    auto on_about_to_remove = [this](QEXTMvvmSessionItem* item, TagRow tagrow) {
...
void ViewModelController::setRootSessionItem(QEXTMvvmSessionItem* item)
{
    if (!item)
...
QEXTMvvmSessionItem* ViewModelController::rootSessionItem() const
{
...
std::vector<ViewItem*> ViewModelController::findViews(const QEXTMvvmSessionItem* item) const
{
...
void ViewModelController::onDataChange(QEXTMvvmSessionItem* item, int role)
{
...
void ViewModelController::onItemInserted(QEXTMvvmSessionItem* parent, TagRow tagrow)
{
    p_impl->insert_view(parent, tagrow);
}

void ViewModelController::onItemRemoved(QEXTMvvmSessionItem*, TagRow) {}

void ViewModelController::onAboutToRemoveItem(QEXTMvvmSessionItem* parent, TagRow tagrow)
{
...
void ViewModelController::update_branch(const QEXTMvvmSessionItem* item)
{
