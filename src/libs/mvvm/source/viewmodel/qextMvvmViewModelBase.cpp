...
struct QEXTMvvmViewModelBase::ViewModelBaseImpl {
    ...
    QScopedPointer<QEXTMvvmViewItem> root;
...
QEXTMvvmViewModelBase::QEXTMvvmViewModelBase(QObject* parent)
    : QAbstractItemModel(parent), p_impl(new ViewModelBaseImpl(this))
{
    beginResetModel();
    setRootViewItem(new QEXTMvvmRootViewItem(QEXT_DECL_NULLPTR));
    endResetModel();
}
...
void QEXTMvvmViewModelBase::insertRow(QEXTMvvmViewItem *parent, int row, QVector<QEXTMvvmViewItem *> items)
{
    if (!p_impl->item_belongs_to_model(parent))
        throw std::runtime_error(
                "Error in QEXTMvvmViewModelBase: attempt to use parent from another model");
...
void QEXTMvvmViewModelBase::appendRow(QEXTMvvmViewItem* parent, QVector<QEXTMvvmViewItem *> items)
{
    insertRow(parent, parent->rowCount(), items);
}
...
void QEXTMvvmViewModelBase::setRootViewItem(QEXTMvvmViewItem *root_item)
{
    p_impl->root.reset(root_item);
}
