...
QEXTMvvmSessionModel::QEXTMvvmSessionModel(std::string QEXTMvvmModelType, std::shared_ptr<QEXTMvvmItemPool> pool)
    : m_model_type(std::move(QEXTMvvmModelType))
    , m_item_manager(std::make_unique<QEXTMvvmItemManager>())
    , m_commands(std::make_unique<QEXTCommandService>(this))
    , m_mapper(std::make_unique<QEXTMvvmModelMapper>(this))
{
    if (pool)
...
void QEXTMvvmSessionModel::setItemCatalogue(std::unique_ptr<QEXTMvvmItemCatalogue> catalogue)
{
    ...
    m_item_manager->setItemFactory(std::make_unique<QEXTMvvmItemFactory>(std::move(full_catalogue)));
}
...
QEXTMvvmVariant QEXTMvvmSessionModel::data(QEXTMvvmSessionItem* item, int role) const
{
    return item->data<QEXTMvvmVariant>(role);
}

bool QEXTMvvmSessionModel::setData(QEXTMvvmSessionItem* item, const QEXTMvvmVariant& value, int role)
{
