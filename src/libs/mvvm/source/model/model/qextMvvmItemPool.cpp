...
identifier_type QEXTMvvmItemPool::register_item(QEXTMvvmSessionItem* item, identifier_type key)
...
identifier_type QEXTMvvmItemPool::key_for_item(QEXTMvvmSessionItem* item) const
{
    auto it = m_item_to_key.find(item);
    if (it != m_item_to_key.end())
        return it->second;
...
QEXTMvvmSessionItem* QEXTMvvmItemPool::item_for_key(const identifier_type& key) const
{
