...
identifier_type ItemPool::register_item(QEXTMvvmSessionItem* item, identifier_type key)
...
void ItemPool::unregister_item(QEXTMvvmSessionItem* item)
{
...
identifier_type ItemPool::key_for_item(QEXTMvvmSessionItem* item) const
{
    auto it = m_item_to_key.find(item);
    if (it != m_item_to_key.end())
        return it->second;
...
QEXTMvvmSessionItem* ItemPool::item_for_key(const identifier_type& key) const
{
