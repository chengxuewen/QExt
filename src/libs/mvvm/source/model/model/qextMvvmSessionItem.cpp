...
struct QEXTMvvmSessionItem::SessionItemImpl {
    ...
    SessionItemImpl(QEXTMvvmSessionItem* this_item)
        ...
        , m_data(std::make_unique<QEXTMvvmSessionItemData>())
        , m_tags(std::make_unique<QEXTMvvmSessionItemTags>())
    {
    }

    //! Sets the data for given role, notifies the model.
    bool setData(const QEXTMvvmVariant& variant, int role)
    {
...
QEXTMvvmSessionItem::QEXTMvvmSessionItem(QEXTMvvmModelType modelType) : p_impl(std::make_unique<SessionItemImpl>(this))
{
...
{
    if (!p_impl->m_mapper)
        p_impl->m_mapper = std::make_unique<QEXTMvvmItemMapper>(this);
...
bool QEXTMvvmSessionItem::set_data_internal(QEXTMvvmVariant value, int role)
{
...
QEXTMvvmVariant QEXTMvvmSessionItem::data_internal(int role) const
{
...
bool QEXTMvvmSessionItem::setDataIntern(const QEXTMvvmVariant& variant, int role)
{
