...
QEXTMvvmStatusLabel::QEXTMvvmStatusLabel(QWidget* parent)
    : QFrame(parent), m_font("Monospace", QEXTMvvmStyle::DefaultInfoBarTextSize(), QFont::Normal, false)
{
    setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);
    setFixedHeight(QEXTMvvmStyle::DefaultInfoBarHeight());
}
...
void QEXTMvvmStatusLabel::paintEvent(QPaintEvent* event)
{
    ...
    const int gap(QEXTMvvmUtils::WidthOfLetterM() / 2); // make it smaller
