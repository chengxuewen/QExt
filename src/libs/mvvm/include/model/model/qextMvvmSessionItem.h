#ifndef _QEXTMVVMSESSIONITEM_H
#define _QEXTMVVMSESSIONITEM_H
...
public:
    ...
    bool setDataIntern(const QEXTMvvmVariant& variant, int role);
    ...
    QEXTMvvmSessionItem* getItem(const std::string& tag, int row = 0) const; // FIXME QEXTMvvmTagRow?
...
private:
    ...
    bool set_data_internal(QEXTMvvmVariant value, int role);
    QEXTMvvmVariant data_internal(int role) const;
    ...
    class QEXTMvvmSessionItemTags* itemTags() const;
...
{
    if constexpr (std::is_same<T, QEXTMvvmVariant>::value)
        return set_data_internal(value, role);
    return set_data_internal(QEXTMvvmVariant::fromValue(value), role);
}
