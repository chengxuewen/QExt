...
class QEXT_MVVM_API QEXTCommandService
{
public:
    QEXTCommandService(QEXTMvvmSessionModel* model);
...
private:
    template <typename C, typename... Args> QEXTCommandResult process_command(Args&&... args);
...
QEXTCommandResult QEXTCommandService::process_command(Args&&... args)
{
    if (provideUndo()) {
