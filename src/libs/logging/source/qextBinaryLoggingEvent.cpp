...
BinaryLoggingEvent::BinaryLoggingEvent(const Logger *logger, QExtLogLevel level, const QByteArray &message)
    : LoggingEvent(logger, level, QString(binMarker))
    , mBinaryMessage(message)
{
}

BinaryLoggingEvent::BinaryLoggingEvent(const Logger *logger, QExtLogLevel level, const QByteArray &message, qint64 timeStamp)
    : LoggingEvent(logger, level, QString(binMarker), timeStamp)
    , mBinaryMessage(message)
{
}

BinaryLoggingEvent::BinaryLoggingEvent(const Logger *logger, QExtLogLevel level, const QByteArray &message, const QString &ndc, const QHash<QString, QString> &properties, const QString &threadName, qint64 timeStamp)
