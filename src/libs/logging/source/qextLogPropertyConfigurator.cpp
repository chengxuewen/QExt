        ...
        LoggerRepository *loggerRepository) const
{
    ...
    if (!value.isNull())
    {
        loggerRepository->setThreshold(OptionConverter::toLevel(value, QExtLogLevel::ALL_INT));
        ...
        LoggerRepository *loggerRepository)
{
    ...
    // - Parse logger data (QExtLogLevel, Appender)
                                       ...
                                       const QString &value)
{
    ...
    if (!sValue.isEmpty())
    {
        ...
        if (sValue.compare(keyword_inherited, Qt::CaseInsensitive) == 0)
            level = QExtLogLevel::NULL_INT;
        else
            level = OptionConverter::toLevel(sValue, QExtLogLevel::DEBUG_INT);
        if (level == QExtLogLevel::NULL_INT && logger->name() == QString())
...
void PropertyConfigurator::startCaptureErrors()
{
    ...
    listAppender->setThreshold(QExtLogLevel::ERROR_INT);
