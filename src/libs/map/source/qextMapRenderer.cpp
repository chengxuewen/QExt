...
#include <private/qextMapRenderer_p.h>
...
MapRenderer::MapRenderer(qreal pixelRatio, Settings::GLContextMode mode, const QString &localFontFamily)
    ...
    // process events.
    if (m_forceScheduler) {
        QMapLibre::Scheduler *scheduler = getScheduler();

        if (mbgl::Scheduler::GetCurrent() == nullptr) {
            mbgl::Scheduler::SetCurrent(scheduler);
        }

        connect(scheduler, &QMapLibre::Scheduler::needsProcessing, this, &MapRenderer::needsRendering);
    }
}
