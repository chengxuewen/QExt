...
#ifndef _QEXTMAPCONVERSION_P_H
#define _QEXTMAPCONVERSION_P_H
...
public:
    ...
    static bool isObject(const QVariant &value)
    {
        ...
        return value.canConvert(QVariant::Map) || value.type() == QVariant::ByteArray
               ...
               value.userType() == qMetaTypeId<QVector<Feature>>() ||
               value.userType() == qMetaTypeId<QList<Feature>>() ||
               value.userType() == qMetaTypeId<std::list<Feature>>();
    }
...
#endif
        return {};_QEXTMAPCONVERSION_P_H
    }

    static std::optional<mbgl::GeoJSON> toGeoJSON(const QVariant &value, Error &error)
    {
        if (value.typeName() == QStringLiteral("QExtMapLibre::Feature")) {
            return mbgl::GeoJSON{QExtMapLibre::GeoJSON::asFeature(value.value<QExtMapLibre::Feature>())};
        }

        if (value.userType() == qMetaTypeId<QVector<QExtMapLibre::Feature>>()) {
            return featureCollectionToGeoJSON(value.value<QVector<QExtMapLibre::Feature>>());
        }

        if (value.userType() == qMetaTypeId<QList<QExtMapLibre::Feature>>()) {
            return featureCollectionToGeoJSON(value.value<QList<QExtMapLibre::Feature>>());
        }

        if (value.userType() == qMetaTypeId<std::list<QExtMapLibre::Feature>>()) {
            return featureCollectionToGeoJSON(value.value<std::list<QExtMapLibre::Feature>>());
        }
...
private:
    ...
    static GeoJSON featureCollectionToGeoJSON(const T &features) {
        ...
        for (const auto &feature : features) {
            collection.push_back(QExtMapLibre::GeoJSON::asFeature(feature));
        }
