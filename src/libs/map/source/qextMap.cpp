...
#include <private/qextMapConversion_p.h>
#include <private/qextMapObserver_p.h>
#include <private/qextMapGeoJson_p.h>
#include <private/qextMap_p.h>
...
static_assert(mbgl::underlying_type(QExtMapLibre::Map::NorthUpwards) ==
                  mbgl::underlying_type(mbgl::NorthOrientation::Upwards),
              "error");
static_assert(mbgl::underlying_type(QExtMapLibre::Map::NorthRightwards) ==
                  mbgl::underlying_type(mbgl::NorthOrientation::Rightwards),
              "error");
static_assert(mbgl::underlying_type(QExtMapLibre::Map::NorthDownwards) ==
                  mbgl::underlying_type(mbgl::NorthOrientation::Downwards),
              "error");
static_assert(mbgl::underlying_type(QExtMapLibre::Map::NorthLeftwards) ==
...
mbgl::MapOptions mapOptionsFromSettings(const QExtMapLibre::Settings &settings, const QSize &size, qreal pixelRatio) {
...
mbgl::ResourceOptions resourceOptionsFromSettings(const QExtMapLibre::Settings &settings) {
...
mbgl::ClientOptions clientOptionsFromSettings(const QExtMapLibre::Settings &settings) {
    return std::move(mbgl::ClientOptions()
                         .withName(settings.clientName().toStdString())
                         .withVersion(settings.clientVersion().toStdString()));
}

std::optional<mbgl::Annotation> asAnnotation(const QExtMapLibre::Annotation &annotation) {
    auto asGeometry = [](const QExtMapLibre::ShapeAnnotationGeometry &geometry) {
        mbgl::ShapeAnnotationGeometry result;
        switch (geometry.type) {
            case QExtMapLibre::ShapeAnnotationGeometry::LineStringType:
                result = QExtMapLibre::GeoJSON::asLineString(geometry.geometry.first().first());
                break;
            case QExtMapLibre::ShapeAnnotationGeometry::PolygonType:
                result = QExtMapLibre::GeoJSON::asPolygon(geometry.geometry.first());
                break;
            case QExtMapLibre::ShapeAnnotationGeometry::MultiLineStringType:
                result = QExtMapLibre::GeoJSON::asMultiLineString(geometry.geometry.first());
                break;
            case QExtMapLibre::ShapeAnnotationGeometry::MultiPolygonType:
                result = QExtMapLibre::GeoJSON::asMultiPolygon(geometry.geometry);
                break;
        }
        return result;
    };

    if (annotation.canConvert<QExtMapLibre::SymbolAnnotation>()) {
        const auto symbolAnnotation = annotation.value<QExtMapLibre::SymbolAnnotation>();
        const QExtMapLibre::Coordinate &pair = symbolAnnotation.geometry;
        return {
            mbgl::SymbolAnnotation(mbgl::Point<double>{pair.second, pair.first}, symbolAnnotation.icon.toStdString())};
    }

    if (annotation.canConvert<QExtMapLibre::LineAnnotation>()) {
        const auto lineAnnotation = annotation.value<QExtMapLibre::LineAnnotation>();
    ...
    if (annotation.canConvert<QExtMapLibre::FillAnnotation>()) {
        const auto fillAnnotation = annotation.value<QExtMapLibre::FillAnnotation>();
...
{
    m_rendererObserver = std::make_shared<QMapLibre::RendererObserver>(*mbgl::util::RunLoop::Get(), observer);
...
bool MapPrivate::setProperty(const PropertySetter &setter,
    ...
    if (value.type() == QVariant::String) {
        ...
        document.Parse<0>(value.toString().toStdString().c_str());
