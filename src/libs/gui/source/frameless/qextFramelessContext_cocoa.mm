...
#include "qextFramelessUtils_p.h"
...
- (void)windowWillEnterFullScreen:(NSNotification *)notification
{
    ...
    if (auto proxy = g_proxyIndexes->value(nswindow))
    {
        reinterpret_cast<QWK_NSWindowDelegate *>(proxy)->windowEvent(QWK_NSWindowDelegate::WillEnterFullScreen);
    }
}
...
- (void)windowWillExitFullScreen:(NSNotification *)notification
{
    ...
    if (auto proxy = g_proxyIndexes->value(nswindow))
    {
        reinterpret_cast<QWK_NSWindowDelegate *>(proxy)->windowEvent(QWK_NSWindowDelegate::WillExitFullScreen);
    }
}
...
- (void)windowDidExitFullScreen:(NSNotification *)notification
{
    ...
    if (auto proxy = g_proxyIndexes->value(nswindow))
    {
        reinterpret_cast<QWK_NSWindowDelegate *>(proxy)->windowEvent(QWK_NSWindowDelegate::DidExitFullScreen);
    }
}
...
- (void)windowDidResize:(NSNotification *)notification
{
    ...
    if (auto proxy = g_proxyIndexes->value(nswindow))
    {
        reinterpret_cast<QWK_NSWindowDelegate *>(proxy)->windowEvent(QWK_NSWindowDelegate::DidResize);
    }
}
...
struct NSWindowProxy : public QWK_NSWindowDelegate
{
    ...
    {
        nswindow = macWindow;
        m_creenRectCallback = QEXT_NULLPTR;
        g_proxyIndexes->insert(nswindow, this);
    }

    ~NSWindowProxy() QEXT_OVERRIDE
    {
        g_proxyIndexes->remove(nswindow);
    }

    // Delegate
    void windowEvent(NSEventType eventType) QEXT_OVERRIDE
    {
        switch (eventType)
        {
            case WillExitFullScreen:
            {
                if (!m_creenRectCallback || !systemButtonVisible)
                {
                    return;
                }
    ...
    void setSystemButtonVisible(bool visible)
    {
        ...
        if (!m_creenRectCallback || !visible)
        {
            return;
        }
        updateSystemButtonRect();
    ...
    // System buttons area
    void setScreenRectCallbackFunc(QExtScreenRectCallbackInterface *callback)
    {
        m_creenRectCallback = callback;
    ...
    void updateSystemButtonRect()
    {
        ...
        QPoint center = m_creenRectCallback->screenRect(QSize(viewSize.width, titlebarHeight)).center();
...
private:
    ...
    QExtScreenRectCallbackInterface *m_creenRectCallback;

    static QWK_NSWindowObserver *windowObserver;

    // NSEvent *lastMouseDownEvent = nil;

    using setStyleMaskPtr = void (*)(id, SEL, NSWindowStyleMask);
    static setStyleMaskPtr oldSetStyleMask;

    using setTitlebarAppearsTransparentPtr = void (*)(id, SEL, BOOL);
    static setTitlebarAppearsTransparentPtr oldSetTitlebarAppearsTransparent;

    using canBecomeKeyWindowPtr = BOOL (*)(id, SEL);
    static canBecomeKeyWindowPtr oldCanBecomeKeyWindow;

    using canBecomeMainWindowPtr = BOOL (*)(id, SEL);
    static canBecomeMainWindowPtr oldCanBecomeMainWindow;

    using sendEventPtr = void (*)(id, SEL, NSEvent *);
    static sendEventPtr oldSendEvent;
};
QWK_NSWindowObserver *NSWindowProxy::windowObserver = QEXT_NULLPTR;
NSWindowProxy::setStyleMaskPtr NSWindowProxy::oldSetStyleMask = QEXT_NULLPTR;
NSWindowProxy::setTitlebarAppearsTransparentPtr NSWindowProxy::oldSetTitlebarAppearsTransparent = QEXT_NULLPTR;
NSWindowProxy::canBecomeKeyWindowPtr NSWindowProxy::oldCanBecomeKeyWindow = QEXT_NULLPTR;
NSWindowProxy::canBecomeMainWindowPtr NSWindowProxy::oldCanBecomeMainWindow = QEXT_NULLPTR;
NSWindowProxy::sendEventPtr NSWindowProxy::oldSendEvent = QEXT_NULLPTR;
...
class CocoaWindowEventFilter : public QExtFramelessSharedEventFilter
{
public:
    explicit CocoaWindowEventFilter(QExtAbstractFramelessContext *context);
    ~CocoaWindowEventFilter() QEXT_OVERRIDE;
...
CocoaWindowEventFilter::CocoaWindowEventFilter(QExtAbstractFramelessContext *context)
...
QExtCocoaFramelessContext::QExtCocoaFramelessContext() : QExtAbstractFramelessContext()
{
    cocoaWindowEventFilter.reset(new CocoaWindowEventFilter(this));
}

QExtCocoaFramelessContext::~QExtCocoaFramelessContext()
{
    releaseWindowProxy(windowId);
}

QString QExtCocoaFramelessContext::key() const
{
    return QStringLiteral("cocoa");
}

void QExtCocoaFramelessContext::virtualHook(int id, void *data)
{
    switch (id)
    {
        case SystemButtonAreaChangedHook:
        {
            ensureWindowProxy(windowId)->setScreenRectCallbackFunc(m_systemButtonAreaCallback.data());
            return;
        }
        default:
            break;
...
QVariant QExtCocoaFramelessContext::windowAttribute(const QString &key) const
{
    ...
    return QExtAbstractFramelessContext::windowAttribute(key);
}

void QExtCocoaFramelessContext::winIdChanged()
{
...
bool QExtCocoaFramelessContext::windowAttributeChanged(const QString &key, const QVariant &attribute,
