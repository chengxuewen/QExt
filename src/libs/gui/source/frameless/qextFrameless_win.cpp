...
QString WindowsRegistryKey::stringValue(QStringView subKey) const
{
    ...
    if (::RegQueryValueExW(m_key, subKeyC, QEXT_NULLPTR, &type, QEXT_NULLPTR, &size) != ERROR_SUCCESS ||
    ...
    if (::RegQueryValueExW(m_key, subKeyC, QEXT_NULLPTR, &type, buffer.data(), &size) == ERROR_SUCCESS)
    {
...
QPair<DWORD, bool> WindowsRegistryKey::dwordValue(QStringView subKey) const
{
    ...
    DWORD type;
    auto subKeyC = reinterpret_cast<const wchar_t *>(subKey.utf16());
    if (::RegQueryValueExW(m_key, subKeyC, QEXT_NULLPTR, &type, QEXT_NULLPTR, QEXT_NULLPTR) != ERROR_SUCCESS ||
            type != REG_DWORD)
    {
        return qMakePair(0, false);
    }
    ...
    const bool ok = ::RegQueryValueExW(m_key, subKeyC, QEXT_NULLPTR, QEXT_NULLPTR,
