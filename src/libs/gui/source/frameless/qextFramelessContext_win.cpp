...
#include "qextFramelessContext_win_p.h"
...
static WNDPROC g_qtWindowProc = QEXT_NULLPTR;
...
{
    ::SetWindowPos(hwnd, QEXT_NULLPTR, 0, 0, 0, 0,
...
static inline void moveWindowToDesktopCenter(HWND hwnd)
{
    ...
    ::SetWindowPos(hwnd, QEXT_NULLPTR, newX, newY, 0, 0,
...
static inline void moveWindowToMonitor(HWND hwnd, const MONITORINFOEXW &activeMonitor)
{
    ...
    ::SetWindowPos(hwnd, QEXT_NULLPTR, newWindowX, newWindowY, RECT_WIDTH(currentWindowRect),
...
static inline void bringWindowToFront(HWND hwnd)
{
    ...
    DWORD windowThreadProcessId = ::GetWindowThreadProcessId(oldForegroundWindow, QEXT_NULLPTR);
...
static void syncPaintEventWithDwm()
{
    ...
    apis.pDwmGetCompositionTimingInfo(QEXT_NULLPTR, &dti);
    LARGE_INTEGER now1{ };
                            ...
                            const bool fixedSize)
{
    ...
    const auto result = ::TrackPopupMenu(
        ...
        pos.x, pos.y, 0, hWnd, QEXT_NULLPTR);
...
class WindowsNativeEventFilter : public QExtFramelessAppNativeEventFilter
{
public:
    ...
    static inline WindowsNativeEventFilter *instance = QEXT_NULLPTR;
    static inline Win32WindowContext *lastMessageContext = QEXT_NULLPTR;
...
WindowsNativeEventFilter::lastMessageContext = QEXT_NULLPTR;
return
result;
}
...
LRESULT result;
if (ctx->
windowProc(hWnd, message, wParam, lParam, &result
)) {
return
result;
}
...
return
::CallWindowProcW(g_qtWindowProc, hWnd, message, wParam, lParam
);
}
...
Win32WindowContext::Win32WindowContext() : QExtAbstractFramelessContext()
{
}
...
void Win32WindowContext::virtualHook(int id, void *data)
{
    switch (id)
    {
        case CentralizeHook:
        ...
        case WM_MOUSELEAVE:
        {
            if (wParam != kMessageTag.wParam)
            {
                ...
                QPoint qtScenePos = QHighDpi::fromNativeLocalPosition(point2qpoint(screenPoint),
                                                                      m_windowHandle);
                auto dummy = QExtFramelessAgent::Unknown;
    ...
    switch (message)
    {
        ...
        case WM_NCHITTEST:
        {
            ...
            [[maybe_unused]] const auto &hitTestRecorder = qScopeGuard([this, result]()
                                                                       {
                                                                           lastHitTestResult = getHitWindowPart(
                                                                               int(*result)); //
                                                                       });
            ...
            QExtFramelessAgent::SystemButton sysButtonType = QExtFramelessAgent::Unknown;
            if (!isFixedSize && isInSystemButtons(qtScenePos, &sysButtonType))
            {
                ...
                *result = HTNOWHERE;
                ...
                if (*result == HTNOWHERE)
                {
                    ...
                    {
                        case QExtFramelessAgent::WindowIcon:
                            *result = HTSYSMENU;
                            break;
                        case QExtFramelessAgent::Help:
                            *result = HTHELP;
                            break;
                        case QExtFramelessAgent::Minimize:
                            *result = HTREDUCE;
                            break;
                        case QExtFramelessAgent::Maximize:
                            *result = HTZOOM;
                            break;
                        case QExtFramelessAgent::Close:
                            *result = HTCLOSE;
                            break;
                        default:
                            // unreachable
    ...
    if (max || full)
    {
        ...
        if (taskbarState & ABS_AUTOHIDE)
        {
            ...
            else
            {
                ...
                APPBARDATA abd2{ };
                abd2.cbSize = sizeof(abd2);
                abd2.hWnd = ::FindWindowW(L"Shell_TrayWnd", QEXT_NULLPTR);
