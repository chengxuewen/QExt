...
#include "ui_navlistwidget.h"
#include "qtimer.h"

NavListWidget::NavListWidget(QWidget *parent) : QWidget(parent), ui(new Ui::NavListWidget)
{
    ui->setupUi(this);
    this->initForm();
}
...
//    m_parentItems4 = ui->navListView4->setItems(items.join(","));
    QList<QEXTNavListWidgetItem *> itemList;
    ...
    QEXTNavListWidgetItem *item = new QEXTNavListWidgetItem(parentItem);
    item->setText("记录查询");
    item->setTip("ss");
    item->setFontIcon(QChar(0xf073));

    item = new QEXTNavListWidgetItem;
    ...
    parentItem = new QEXTNavListWidgetItem;
    itemList.append(parentItem);
...
//    ui->navListView4->setModel(ui->navListView3->model());
    ...
    ui->navListView1->setExpendMode(QEXTNavListWidget::ExpendMode_SingleClick);
    ui->navListView2->setExpendMode(QEXTNavListWidget::ExpendMode_DoubleClick);
    ui->navListView3->setExpendMode(QEXTNavListWidget::ExpendMode_SingleClick);
    ui->navListView4->setExpendMode(QEXTNavListWidget::ExpendMode_SingleClick);
    connect(ui->navListView1, SIGNAL(itemPressed(QEXTNavListWidgetItem*)), this, SLOT(onItemPressed1(QEXTNavListWidgetItem*)));
    connect(ui->navListView2, SIGNAL(itemPressed(QEXTNavListWidgetItem*)), this, SLOT(onItemPressed2(QEXTNavListWidgetItem*)));
    connect(ui->navListView3, SIGNAL(itemPressed(QEXTNavListWidgetItem*)), this, SLOT(onItemPressed3(QEXTNavListWidgetItem*)));
    connect(ui->navListView4, SIGNAL(itemPressed(QEXTNavListWidgetItem*)), this, SLOT(onItemPressed4(QEXTNavListWidgetItem*)));
}

void NavListWidget::onItemPressed1(QEXTNavListWidgetItem *item)
{
    if (item->isChildItem())
    {
...
void NavListWidget::onItemPressed2(QEXTNavListWidgetItem *item)
{
    if (item->isChildItem())
    {
//        item->setEnable(false);
...
void NavListWidget::onItemPressed3(QEXTNavListWidgetItem *item)
{
    if (item->isChildItem())
    {
//        item->setEnable(false);
...
void NavListWidget::onItemPressed4(QEXTNavListWidgetItem *item)
{
    if (item->isChildItem())
    {
