...
QEXTFramelessDialog::QEXTFramelessDialog(QWidget *parent) : QDialog(parent)
{
    m_padding = 8;
    m_moveEnable = true;
    m_resizeEnable = true;
...
void QEXTFramelessDialog::doWindowStateChange(QEvent *event)
{
    ...
    emit windowStateChanged(!m_moveEnable);
    ...
    if (eventType == "windows_generic_MSG") {
        ...
        if (msg->message == WM_NCCALCSIZE) {
            *result = 0;
            ...
            bool right = pos.x() > width() - m_padding;
            bool top = pos.y() < padding;
            bool bottom = pos.y() > height() - m_padding;
