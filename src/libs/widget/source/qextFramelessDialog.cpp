...
QEXTFramelessDialogPrivate::QEXTFramelessDialogPrivate(QEXTFramelessDialog *q)
    : q_ptr(q)
{
    m_padding = 8;
    m_moveEnable = true;
...
QEXTFramelessDialogPrivate::~QEXTFramelessDialogPrivate()
{
...
QEXTFramelessDialog::~QEXTFramelessDialog()
{

}

void QEXTFramelessDialog::showEvent(QShowEvent *event)
{
...
void QEXTFramelessDialog::doWindowStateChange(QEvent *event)
{
    ...
    emit windowStateChanged(!d->m_moveEnable);
    ...
    if (windowState() & Qt::WindowMinimized) {
        ...
        if (m_isMin) {
            //设置无边框属性
            this->setWindowFlags(m_flags | Qt::FramelessWindowHint);
    ...
    if (eventType == "windows_generic_MSG") {
        ...
        if (msg->message == WM_NCCALCSIZE) {
            *result = 0;
            ...
            QPoint pos = this->mapFromGlobal(QPoint(x, y));

            //判断当前鼠标位置在哪个区域
            bool left = pos.x() < d->m_padding;
            bool right = pos.x() > this->width() - d->m_padding;
            bool top = pos.y() < padding;
            bool bottom = pos.y() > this->height() - d->m_padding;
...
#ifdef Q_OS_WIN
bool QEXTFramelessDialog::winEvent(MSG *message, long *result)
{
