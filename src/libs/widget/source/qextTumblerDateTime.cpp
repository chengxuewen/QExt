...
#include <qextTumblerDateTime.h>
#include <qextTumbler.h>

#include <QBoxLayout>
#include <QDebug>

QEXTTumblerDateTime::QEXTTumblerDateTime(QWidget *parent) : QWidget(parent)
{
    this->initForm();
}

void QEXTTumblerDateTime::initForm()
{
    //年份选择器
    tumblerYear = new QEXTTumbler(this);
    ...
    //月份选择器
    tumblerMonth = new QEXTTumbler(this);
    ...
    //日期选择器
    tumblerDay = new QEXTTumbler(this);
    QStringList listDay;
    for (int i = 1; i <= 31; i++) {
    ...
    //时钟选择器
    tumblerHour = new QEXTTumbler(this);
    ...
    //分钟选择器
    tumblerMin = new QEXTTumbler(this);
    QStringList listMin;
    for (int i = 0; i <= 59; i++) {
    ...
    //秒钟选择器
    tumblerSec = new QEXTTumbler(this);
    QStringList listSec;
    for (int i = 0; i <= 59; i++) {
...
void QEXTTumblerDateTime::currentValueChanged(const QString &)
{
...
int QEXTTumblerDateTime::getYear() const
{
    return tumblerYear->getCurrentValue().toInt();
}

int QEXTTumblerDateTime::getMonth() const
{
    return tumblerMonth->getCurrentValue().left(2).toInt();
}

int QEXTTumblerDateTime::getDay() const
{
    return tumblerDay->getCurrentValue().toInt();
}

int QEXTTumblerDateTime::getHour() const
{
    return tumblerHour->getCurrentValue().toInt();
}

int QEXTTumblerDateTime::getMin() const
{
    return tumblerMin->getCurrentValue().toInt();
}

int QEXTTumblerDateTime::getSec() const
{
    return tumblerSec->getCurrentValue().toInt();
}

void QEXTTumblerDateTime::setYear(int year)
{
    tumblerYear->setCurrentValue(QString("%1").arg(year));
}

void QEXTTumblerDateTime::setMonth(int month)
{
    tumblerMonth->setCurrentValue(QString("%1 月").arg(month));
}

void QEXTTumblerDateTime::setDay(int day)
{
    tumblerDay->setCurrentValue(QString("%1").arg(day));
}

void QEXTTumblerDateTime::setHour(int hour)
{
    tumblerHour->setCurrentValue(QString("%1").arg(hour));
}

void QEXTTumblerDateTime::setMin(int min)
{
    tumblerMin->setCurrentValue(QString("%1").arg(min));
}

void QEXTTumblerDateTime::setSec(int sec)
{
    tumblerSec->setCurrentValue(QString("%1").arg(sec));
}

void QEXTTumblerDateTime::setDateTime(int year, int month, int day, int hour, int min, int sec)
{
    setYear(year);
    setMonth(month);
    setDay(day);
