...
void MainWindow::createToolBox()
{
    ...
    QExtGraphicsRectItem item(QRect(0,0,48,48));
    QIcon icon(item.image());
    QExtGraphicsEllipseItem item1(QRect(0,0,48,48));
...
{
    if ( sender() == selectAct )
        QExtGraphicsTool::sm_drawShape = selection;
    else if (sender() == lineAct )
        QExtGraphicsTool::sm_drawShape = line;
    else if ( sender() == rectAct )
        QExtGraphicsTool::sm_drawShape = rectangle;
    else if ( sender() == roundRectAct )
        QExtGraphicsTool::sm_drawShape = roundrect;
    else if ( sender() == ellipseAct )
        QExtGraphicsTool::sm_drawShape = ellipse ;
    else if ( sender() == polygonAct )
        QExtGraphicsTool::sm_drawShape = polygon;
    else if ( sender() == bezierAct )
        QExtGraphicsTool::sm_drawShape = bezier ;
    else if (sender() == rotateAct )
        QExtGraphicsTool::sm_drawShape = rotation;
    else if (sender() == polylineAct )
        QExtGraphicsTool::sm_drawShape = polyline;
...
void MainWindow::updateActions()
{
    ...
    selectAct->setChecked(QExtGraphicsTool::sm_drawShape == selection);
    lineAct->setChecked(QExtGraphicsTool::sm_drawShape == line);
    rectAct->setChecked(QExtGraphicsTool::sm_drawShape == rectangle);
    roundRectAct->setChecked(QExtGraphicsTool::sm_drawShape == roundrect);
    ellipseAct->setChecked(QExtGraphicsTool::sm_drawShape == ellipse);
    bezierAct->setChecked(QExtGraphicsTool::sm_drawShape == bezier);
    rotateAct->setChecked(QExtGraphicsTool::sm_drawShape == rotation);
    polygonAct->setChecked(QExtGraphicsTool::sm_drawShape == polygon);
    polylineAct->setChecked(QExtGraphicsTool::sm_drawShape == polyline );
    ...
    unGroupAct->setEnabled(scene &&scene->selectedItems().count() > 0 &&
                           dynamic_cast<QExtGraphicsItemGroup*>( scene->selectedItems().first()));
...
void MainWindow::itemMoved(QGraphicsItem *item, const QPointF &oldPosition)
{
    ...
    if ( item ){
        QUndoCommand *moveCommand = new QExtGraphicsMoveShapeCommand(item, oldPosition);
        undoStack->push(moveCommand);
    }else{
        QUndoCommand *moveCommand = new QExtGraphicsMoveShapeCommand(activeMdiChild()->scene(), oldPosition);
        undoStack->push(moveCommand);
    }
}
...
void MainWindow::itemAdded(QGraphicsItem *item)
{
    ...
    QUndoCommand *addCommand = new QExtGraphicsAddShapeCommand(item, item->scene());
...
void MainWindow::itemRotate(QGraphicsItem *item, const qreal oldAngle)
{
    ...
    QUndoCommand *rotateCommand = new QExtGraphicsRotateShapeCommand(item , oldAngle);
...
void MainWindow::itemResize(QGraphicsItem *item, int handle, const QPointF& scale)
{
    ...
    QUndoCommand *resizeCommand = new QExtGraphicsResizeShapeCommand(item ,handle, scale);
...
void MainWindow::itemControl(QGraphicsItem *item, int handle, const QPointF & newPos ,const QPointF &lastPos_)
{
    ...
    QUndoCommand *controlCommand = new QExtGraphicsControlShapeCommand(item ,handle, newPos, lastPos_ );
...
void MainWindow::deleteItem()
{
    ...
    QUndoCommand *deleteCommand = new QExtGraphicsRemoveShapeCommand(scene);
...
void MainWindow::on_actionBringToFront_triggered()
{
    ...
    foreach (QGraphicsItem *item, overlapItems) {
        if (item->zValue() >= zValue && item->type() == QExtGraphicsItem::Type)
...
{
    if (!activeMdiChild()) return ;
    QExtGraphicsScene * scene =dynamic_cast<QExtGraphicsScene*>(activeMdiChild()->scene());
...
void MainWindow::on_group_triggered()
{
    ...
    QExtGraphicsItemGroup *group = scene->createGroup(selectedItems);
    QUndoCommand *groupCommand = new QExtGraphicsGroupShapeCommand(group,scene);
...
void MainWindow::on_unGroup_triggered()
{
    ...
    QGraphicsItem *selectedItem = scene->selectedItems().first();
    QExtGraphicsItemGroup * group = dynamic_cast<QExtGraphicsItemGroup*>(selectedItem);
    if ( group ){
        QUndoCommand *unGroupCommand = new QExtGraphicsUnGroupShapeCommand(group,scene);
...
void MainWindow::on_copy()
{
    ...
    QExtGraphicsShapeMimeData * data = new QExtGraphicsShapeMimeData( scene->selectedItems() );
...
void MainWindow::on_paste()
{
    ...
    QExtGraphicsShapeMimeData * data = dynamic_cast< QExtGraphicsShapeMimeData*>( mp );
    if ( data ){
        ...
        foreach (QGraphicsItem * item , data->items() ) {
            ...
            if ( copy ){
                ...
                copy->moveBy(10,10);
                QUndoCommand *addCommand = new QExtGraphicsAddShapeCommand(copy, scene);
                undoStack->push(addCommand);
            }
        }
    }
...
void MainWindow::on_cut()
{
    ...
    undoStack->push(deleteCommand);
    if ( copylist.count() > 0 ){
        QExtGraphicsShapeMimeData * data = new QExtGraphicsShapeMimeData( copylist );
        QApplication::clipboard()->setMimeData(data);
    }
}
