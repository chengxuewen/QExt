...
//        qDebug() << "### pushbutton state " << (int)opt->state;
        ...
        p->setPen(QPen(getColor(opt, DPalette::FrameBorder, w), dstyle::Metrics::Painter_PenWidth));
        p->setBrush(Qt::NoBrush);
        const QMarginsF border_margins(dstyle::Painter_PenWidth, dstyle::Painter_PenWidth, dstyle::Painter_PenWidth, dstyle::Painter_PenWidth);
                                 ...
                                 QPainter *p, const QWidget *w) const
{
    ...
    case CE_TabBarTab: {
        if (const QStyleOptionTab *tab = qstyleoption_cast<const QStyleOptionTab*>(opt)) {
            ...
            if (!type_check) {
                btn.rect.adjust(dstyle::TabBar_TabMargin / 2, 0, -(dstyle::TabBar_TabMargin / 2), 0);
            }
            btn.state = tab->state;
            ...
            newTab->rect.adjust(dstyle::TabBar_TabMargin / 2, 0, -(dstyle::TabBar_TabMargin / 2), 0);
...
bool ChameleonStyle::drawTabBarScrollButton(QPainter *painter, const QStyleOption *opt, const QWidget *buttonWidget) const
{
    ...
    painter->setPen(QPen(getColor(opt, DPalette::FrameBorder, buttonWidget), dstyle::Metrics::Painter_PenWidth));
    painter->setBrush(lg);
...
//    const bool hasFocus(cb->state & QStyle::State_HasFocus);
    ...
    contentsRect.adjust(dstyle::Metrics::Layout_ChildMarginWidth, 0, -dstyle::Metrics::Layout_ChildMarginWidth - DStyle::pixelMetric(PM_FrameRadius), 0);
    QSize iconSize;
    if (hasIcon) {
    ...
    if (hasText && !hasIcon) {
        ...
        const int contentsWidth(iconSize.width() + textSize.width() + dstyle::Metrics::Button_ItemSpacing);
        ...
        textRect = QRect(QPoint(iconRect.right() + dstyle::Metrics::Button_ItemSpacing + 1,
...
bool ChameleonStyle::drawMenuItem(const QStyleOptionMenuItem *option, QPainter *painter, const QWidget *widget) const
{
    if (const QStyleOptionMenuItem *menuItem = option) {
        ...
        QRect textRect(xpos, y + dstyle::Menu_ItemHTextMargin, w - xpos - tab, h - 2 * dstyle::Menu_ItemVTextMargin);
        ...
        if (!textRef.isEmpty()) {
            ...
            if (tabIndex >= 0) {
                ...
                vShortcutStartPoint.setX(vShortcutStartPoint.x() - dstyle::Menu_PanelRightPadding - realMargins);
            ...
            case SC_ComboBoxListBoxPopup: {
                ...
                if (auto styopt = static_cast<const QStyleOption *>(opt)) {
                    if (auto menuItem = static_cast<const QStyleOptionMenuItem *>(styopt)) {
                        if (menuItem->icon.isNull()) {
                            rect.setWidth(size.width() + dstyle::Menu_CheckMarkWidth);
                        }
                    }
                }
                return rect;
                                       ...
                                       const QSize &contentsSize, const QWidget *widget) const
{
    ...
    case CT_TabBarTab: {
        if (const QStyleOptionTab *tab = qstyleoption_cast<const QStyleOptionTab*>(opt)) {
            ...
            int frame_radius = DStyle::pixelMetric(PM_FrameRadius, opt, widget);
            size.rwidth() += 2 * frame_radius + proxy()->pixelMetric(PM_TabCloseIndicatorWidth, opt, widget) + dstyle::Metrics::TabBar_TabMargin;
        }
        Q_FALLTHROUGH();
    }
    case CT_PushButton: {
    ...
    case CT_MenuItem: {
        if (const QStyleOptionMenuItem *menuItem = qstyleoption_cast<const QStyleOptionMenuItem *>(opt)) {
            ...
            int tabSpacing = dstyle::MenuItem_TabSpacing;
            if (menuItem->text.contains(QLatin1Char('\t'))) {  //若是项有快捷键，文本内容会以'\t'连接快捷键文本
                if (!hideShortcutText)
                    m_width += tabSpacing;
            } else {
                if (menuItem->menuItemType == QStyleOptionMenuItem::SubMenu) {
                    m_width += 2 * dstyle::Menu_ArrowHMargin;
                }
            }
        ...
        size.setHeight(size.height() + qMax(dstyle::Menu_ItemVMargin * 2, 0));
        break;
    }
    case CT_ScrollBar: {
        if (size.width() > size.height())
            size.setHeight(dstyle::ScrollBar_SliderWidth);
        if (size.width() < size.height())
            size.setWidth(dstyle::ScrollBar_SliderWidth);
        return size;
    }
    case CT_RadioButton:
    case CT_CheckBox: {
                                ...
                                const QWidget *widget) const
{
    ...
    case PM_SpinBoxSliderHeight:
        return dstyle::SpinBox_MiniHeight;
    case PM_SpinBoxFrameWidth:
        return dstyle::SpinBox_FrameWidth;
    ...
    case PM_TabBarTabOverlap:
        return dstyle::TabBar_TabOverlap;
    case PM_TabBarBaseOverlap:
        return dstyle::TabBar_BaseOverlap;
    ...
    case PM_MenuScrollerHeight:
        return 10 + dstyle::Metrics::Frame_FrameWidth ;
    ...
    case PM_ComboBoxFrameWidth: { //这是ComboBox VMargin
        ...
        return comboBoxOption && comboBoxOption->editable ? dstyle::Metrics::ComboBox_FrameWidth : dstyle::Metrics::LineEdit_FrameWidth ;
    }
    case PM_MenuVMargin:
        return 8;
    case PM_MenuHMargin:
