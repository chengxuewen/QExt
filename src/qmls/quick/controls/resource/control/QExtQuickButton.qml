...
Button {
    ...
    onPressed: {
        qextButton.state = QExtQuickControls.stateToString(QExtQuickControls.State_Pressed)
        if (pressPromptEnable) {
            scale = 0.95
        }
    }
    onReleased: {
        qextButton.state = QExtQuickControls.stateToString(QExtQuickControls.State_Hovered)
        if (pressPromptEnable) {
            scale = 1
        }
    }
    onHoveredChanged: {
        if (qextButton.hovered) {
            ...
            qextButton.state = QExtQuickControls.stateToString(QExtQuickControls.State_Normal)
        }
    }
    onTextFontChanged: {
