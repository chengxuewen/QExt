...
QExtQuickPopup {
    ...
    function openToGlobal(item , off_x , off_y){
        ...
        bodyLoader.x = p.x - root.rootWindow.x;
        bodyLoader.y = p.y - root.rootWindow.y;
    ...
    function resetLayout(){
        if(bodyLoader.width + bodyLoader.x > root.rootWindow.width){
            bodyLoader.x = root.rootWindow.width - bodyLoader.width - 5;
        }
        else if(bodyLoader.x <= 0){
            bodyLoader.x = 5;
        }

        if(bodyLoader.height + bodyLoader.y > root.rootWindow.height){
            bodyLoader.y = root.rootWindow.height - bodyLoader.height - 5;
