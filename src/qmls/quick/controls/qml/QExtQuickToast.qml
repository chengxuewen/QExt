...
QExtQmlObject {
    ...
    property alias padding: mPaddingInfo
    ...
    function checkShowAvailable() {
        if (!rootWindow) {
            console.warn("rootWindow null, can not show toast!")
            return false;
        }
        return true;
    }
    function showSuccess(text, duration, moremsg, closeable) {
        if (checkShowAvailable()) {
            closeable = (undefined === closeable) ? mControl.defaultCloseable : closeable;
    ...
    QExtQmlObject {
        ...
        function initScreenLayout() {
            if (null == screenLayout) {
                screenLayout = mScreenlayoutComponent.createObject(rootWindow);
                screenLayout.z = Number.MAX_VALUE;
            }
        }

        Component {
            id: mScreenlayoutComponent
            Column {
                ...
                function updatePos() {
                    switch (mControl.verticalAlignment) {
                    ...
                    case Qt.AlignBottom:
                    {
                        y = parentHeight - height - mControl.padding.bottomPadding;
                        break;
                    }
                    case Qt.AlignTop:
                    default:
                    {
                        y = mControl.padding.topPadding;
                        break;
                    }
                    }
                }
        ...
        Component {
            ...
            Item {
                ...
                Loader {
                    ...
                    function updatePos() {
                        switch (mControl.horizontalAlignment) {
                        case Qt.AlignHCenter:
                            x = (parentWidth - width) / 2
                            break
                        case Qt.AlignRight:
                            x = parentWidth - width - mControl.padding.rightPadding
                            break
                        case Qt.AlignLeft:
                        default:
                            x = mControl.padding.leftPadding
                            break
                        }
                    }
                }
                ...
                Column {
                    ...
                    Text {
                        ...
                        onImplicitWidthChanged: {
                            width = implicitWidth < mData.maxWidth - 100 ? 220 : mData.maxWidth
                        }
                    }
                }
            }
